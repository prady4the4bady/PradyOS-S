"""Phase 154 — tests for the /api/v1/fibonacci endpoints in sovereign_web."""
...
def _insert(client, value):
    return client.post("/api/v1/fibonacci/insert", json={"value": value}).json()["handle"]


# ── insert ────────────────────────────────────────────────────────────────────────────────

def test_insert_returns_handle(client):
    body = client.post("/api/v1/fibonacci/insert", json={"value": 5}).json()
    assert "handle" in body and body["size"] == 1 and body["min"] == 5


def test_insert_missing_422(client):
    assert client.post("/api/v1/fibonacci/insert", json={}).status_code == 422


def test_insert_non_num_422(client):
    resp = client.post("/api/v1/fibonacci/insert", json={"value": "x"})
...
def test_find_min(client):
    for v in (8, 3, 9, 1):
        _insert(client, v)
    assert client.get("/api/v1/fibonacci/find_min").json()["min"] == 1


def test_find_min_empty_null(client):
    body = client.get("/api/v1/fibonacci/find_min").json()
...
def test_extract_min(client):
    ...
    body = client.post("/api/v1/fibonacci/extract_min").json()
    assert body["min"] == 2 and body["size"] == 1


def test_extract_min_empty_422(client):
    resp = client.post("/api/v1/fibonacci/extract_min")
    assert resp.status_code == 422 and "error" in resp.json()
...
def test_drain_sorted_over_http(client):
    for v in (5, 3, 8, 1, 9, 2):
        _insert(client, v)
    drained = [client.post("/api/v1/fibonacci/extract_min").json()["min"] for _ in range(6)]
...
def test_decrease_key_to_new_min(client):
    ...
    body = client.post("/api/v1/fibonacci/decrease_key", json={"handle": h, "value": 1}).json()
    assert body["min"] == 1


def test_decrease_key_missing_422(client):
    assert client.post("/api/v1/fibonacci/decrease_key", json={"handle": 0}).status_code == 422


def test_decrease_key_increase_422(client):
    h = _insert(client, 5)
    resp = client.post("/api/v1/fibonacci/decrease_key", json={"handle": h, "value": 9})
    assert resp.status_code == 422 and "error" in resp.json()


def test_decrease_key_bad_handle_422(client):
    resp = client.post("/api/v1/fibonacci/decrease_key", json={"handle": 999, "value": 1})
    assert resp.status_code == 422 and "error" in resp.json()
...
def test_stats_keys(client):
    assert set(client.get("/api/v1/fibonacci/stats").json()) == {"size", "num_trees", "min"}


def test_stats_after_insert(client):
    _insert(client, 7); _insert(client, 3)
    s = client.get("/api/v1/fibonacci/stats").json()
    assert s["size"] == 2 and s["min"] == 3


def test_reset_clears(client):
    _insert(client, 1); _insert(client, 2)
    body = client.request("DELETE", "/api/v1/fibonacci/reset").json()
...
def test_full_workflow(client):
    ...
    _insert(client, 50)
    client.post("/api/v1/fibonacci/decrease_key", json={"handle": h1, "value": 1})
    assert client.get("/api/v1/fibonacci/find_min").json()["min"] == 1
    assert client.post("/api/v1/fibonacci/extract_min").json()["min"] == 1
    assert client.get("/api/v1/fibonacci/find_min").json()["min"] == 50
...
def test_prior_phase_routes_still_live(client):
    ...
    for stats in ("cuckoo", "minhash", "quotient", "kll", "theta", "countsketch",
                  ...
                  "suffixautomaton", "veb", "prquadtree"):
