"""Phase 156 — tests for the /api/v1/btree endpoints in sovereign_web."""
from __future__ import annotations

import pytest
from fastapi.testclient import TestClient

from pradyos.core.b_tree import BTree
...
def filled_client():
    ...
    for k in (50, 30, 70, 20, 40, 60, 80, 10, 90):
        bt.insert(k)
    return TestClient(create_app(b_tree=bt))


# ── insert ────────────────────────────────────────────────────────────────────────────────

def test_insert_returns_added(client):
    body = client.post("/api/v1/btree/insert", json={"key": 5}).json()
    assert body["added"] is True and body["size"] == 1


def test_insert_duplicate_not_added(client):
    client.post("/api/v1/btree/insert", json={"key": 5})
    body = client.post("/api/v1/btree/insert", json={"key": 5}).json()
    assert body["added"] is False and body["size"] == 1


def test_insert_missing_422(client):
    assert client.post("/api/v1/btree/insert", json={}).status_code == 422


def test_insert_unorderable_422(client):
    resp = client.post("/api/v1/btree/insert", json={"key": [1, 2]})
...
def test_delete_present(filled_client):
    body = filled_client.post("/api/v1/btree/delete", json={"key": 30}).json()
    assert body["deleted"] is True and body["size"] == 8


def test_delete_absent(filled_client):
    body = filled_client.post("/api/v1/btree/delete", json={"key": 999}).json()
    assert body["deleted"] is False and body["size"] == 9


def test_delete_missing_422(client):
    assert client.post("/api/v1/btree/delete", json={}).status_code == 422


# ── contains ─────────────────────────────────────────────────────────────────────────────

def test_contains_true(filled_client):
    assert filled_client.get("/api/v1/btree/contains", params={"key": 40}).json()["contains"] is True


def test_contains_false(filled_client):
    assert filled_client.get("/api/v1/btree/contains", params={"key": 45}).json()["contains"] is False


# ── keys ─────────────────────────────────────────────────────────────────────────────────

def test_keys_sorted(filled_client):
    body = filled_client.get("/api/v1/btree/keys").json()
    assert body["keys"] == [10, 20, 30, 40, 50, 60, 70, 80, 90] and body["size"] == 9


def test_keys_empty(client):
    body = client.get("/api/v1/btree/keys").json()
...
def test_stats_keys(client):
    assert set(client.get("/api/v1/btree/stats").json()) == {"size", "height", "min_degree", "min", "max"}


def test_stats_values(filled_client):
    s = filled_client.get("/api/v1/btree/stats").json()
    assert s["size"] == 9 and s["min"] == 10 and s["max"] == 90 and s["min_degree"] == 2


def test_reset_clears(filled_client):
    body = filled_client.request("DELETE", "/api/v1/btree/reset").json()
...
def test_full_workflow(client):
    for k in (10, 20, 30):
        client.post("/api/v1/btree/insert", json={"key": k})
    assert client.get("/api/v1/btree/contains", params={"key": 20}).json()["contains"] is True
    assert client.get("/api/v1/btree/keys").json()["keys"] == [10, 20, 30]
    client.post("/api/v1/btree/delete", json={"key": 20})
    assert client.get("/api/v1/btree/contains", params={"key": 20}).json()["contains"] is False
    assert client.get("/api/v1/btree/keys").json()["keys"] == [10, 30]
...
def test_prior_phase_routes_still_live(client):
    ...
    for stats in ("cuckoo", "minhash", "quotient", "kll", "theta", "countsketch",
                  ...
                  "suffixautomaton", "veb", "prquadtree", "fibonacci", "avl"):
