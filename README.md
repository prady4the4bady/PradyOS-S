...
**Phase 54 — Planned.** Sovereign RetryPolicy — configurable retry execution with exponential back-off, jitter, and per-exception filtering; RetryPolicy(max_attempts=3, base_delay=1.0, backoff_factor=2.0, jitter=0.1, retry_on=(Exception,)); execute(name, fn, *args, **kwargs) runs fn up to max_attempts times, sleeping base_delay * backoff_factor^attempt + uniform(-jitter,jitter) between attempts, records each attempt as an AttemptRecord(name, attempt, outcome, elapsed, error); get_history(name) returns list of AttemptRecords; clear_history(name); list_policies(); exposes GET /api/v1/retry (list all policy names + attempt counts), POST /api/v1/retry/execute (body={name, fn_key} — fn_key maps to a registered no-op for testing), GET /api/v1/retry/{name}/history; stdlib only; 20 unit + 10 web tests.
