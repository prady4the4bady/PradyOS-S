...
**Phase 38 — Planned.** Sovereign Scheduler — the OS schedules its own tasks: a TaskScheduler holds a registry of named tasks, each with an interval_seconds, a callable, and an optional run_at (next scheduled time); calling tick() advances the clock — for each task whose run_at <= now, the task callable is called, its last_run and next run_at are updated, and a TaskRun record (task_name, ran_at, duration_ms, success: bool, error: str|None) is appended to a ring buffer (max 1000); tick() returns list of TaskRun for this tick; exposes GET /api/v1/scheduler/tasks (list tasks with next_run_at and last_run), POST /api/v1/scheduler/tasks (register: name, interval_seconds), DELETE /api/v1/scheduler/tasks/{name}, POST /api/v1/scheduler/tick (advance clock once, returns TaskRun list); stdlib only — no background threads; tick() is meant to be called by an external heartbeat (later wired to the OS loop).
