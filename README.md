...
**Phase 65 — Planned.** Sovereign Query Bus — mirrors Command Bus on the read side; named query handlers registered at runtime; each handler is a callable(params: dict) -> dict; QueryBus.query(name, params) dispatches, records a QueryResult(query_name, params, result, queried_at, duration_ms, success, error); ring-buffer history (max 500, newest-first); exposes GET /api/v1/queries/handlers, POST /api/v1/queries/execute (body: {name, params}), GET /api/v1/queries/history?limit=N, DELETE /api/v1/queries/handlers/{name}; stdlib only; thread-safe.
