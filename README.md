...
**Phase 53 — Planned.** Sovereign CircuitBreaker — per-service failure tracking with automatic open/half-open/closed state transitions; CircuitBreaker(name, failure_threshold=5, recovery_timeout=30, half_open_max=1); states: CLOSED (normal), OPEN (rejecting calls), HALF_OPEN (allowing probe calls); call(name, fn, *args, **kwargs) executes fn if CLOSED or HALF_OPEN, records success/failure, transitions state accordingly — raises CircuitOpenError immediately if OPEN; get_state(name) returns {name, state, failure_count, last_failure_at, opened_at}; reset(name) forces back to CLOSED; list_breakers() returns all; exposes GET /api/v1/breakers (list all), POST /api/v1/breakers (register: body={name, failure_threshold?, recovery_timeout?, half_open_max?}), GET /api/v1/breakers/{name} (state), POST /api/v1/breakers/{name}/reset; stdlib only; 20 unit + 10 web tests.
