...
**Phase 48 — Planned.** Sovereign EventSourcing — append-only event store with projections; EventStore.append(stream: str, event_type: str, payload: dict) -> Event (id=uuid4 hex, stream, event_type, payload, sequence: int auto-increment per stream, occurred_at: float); EventStore.read(stream: str, from_seq: int = 0) -> list[Event]; EventStore.project(stream: str, reducer: Callable[[dict, Event], dict], initial: dict = {}) -> dict — fold all events from_seq=0 into a state dict; persists to JSONL via SnapshotStore (namespace='event_store', key=stream); exposes POST /api/v1/events/{stream}, GET /api/v1/events/{stream}?from_seq=N, POST /api/v1/events/{stream}/project with body {initial: dict, reducer_steps: list[{match_type: str, updates: dict}]} (a simple declarative reducer — if event_type matches, merge updates into state); stdlib only; 20 unit + 10 web tests.
