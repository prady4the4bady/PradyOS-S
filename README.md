...
**Phase 56 — Planned.** Sovereign TimeoutGuard — per-call deadline enforcement; TimeoutGuard wraps any callable and enforces a wall-clock deadline (in seconds) using concurrent.futures.ThreadPoolExecutor with a single worker; TimeoutGuard(default_timeout=5.0); execute(name, fn, *args, timeout=None, **kwargs) — uses timeout param if provided, else default_timeout; raises TimeoutExpiredError (subclass of RuntimeError) if the call does not complete within the deadline; records each call as a GuardRecord(name, timeout, elapsed, outcome, error, ts) where outcome is 'success' | 'timeout' | 'error'; get_history(name), clear_history(name), list_names(), count(name=None); exposes GET /api/v1/timeouts (list names + counts), POST /api/v1/timeouts/execute (body={name, sleep?, timeout?, should_error?} — submits a configurable-sleep/error no-op), GET /api/v1/timeouts/{name}/history, DELETE /api/v1/timeouts/{name}/history; stdlib only (concurrent.futures allowed); 20 unit + 10 web tests.
