...
**Phase 70 — Planned.** Sovereign Anomaly Watch — a tick-driven background loop that periodically runs the Phase 69 `AnomalyDetector` across every registered `SignalAggregator` signal, debounces repeat alerts, and publishes `"anomaly.detected"` bus events on `normal → warning` / `warning → critical` severity transitions; `AnomalyWatch(detector, bus, signals=None, window=3600.0)` with `tick(now=None)` (returns the list of signals whose severity changed), `register_signal(name)`, `last_severity(name)`, `snapshot()`; exposes `GET /api/v1/anomaly/watch` (per-signal current severity) and `POST /api/v1/anomaly/watch/tick`; stdlib only; thread-safe via `threading.Lock`.
