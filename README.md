...
**Phase 61 — Planned.** Sovereign TagIndex — multi-value tag store; TagIndex holds a dict[str, set[str]] mapping tag → set of item IDs; tag(item_id, *tags) adds the item to each tag's set; untag(item_id, *tags) removes; items(tag) → sorted list of IDs bearing that tag; tags(item_id) → sorted list of tags on that item; delete_item(item_id) removes from all tags; search(*tags, mode='all') → sorted IDs bearing ALL (mode=all) or ANY (mode=any) of the given tags; thread-safe via threading.Lock; exposes GET /api/v1/tags (list all tags + counts), POST /api/v1/tags/tag (body={item_id, tags:[]}), POST /api/v1/tags/untag (body={item_id, tags:[]}), GET /api/v1/tags/items/{tag} (items bearing tag), GET /api/v1/tags/search?tags=a,b&mode=all|any, DELETE /api/v1/tags/items/{item_id}; stdlib only; 20 unit + 10 web tests.
