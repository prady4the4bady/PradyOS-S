...
**Phase 60 — Planned.** Sovereign PipelineChain — composable, named processing pipeline for event dicts; a PipelineChain holds an ordered list of Steps, each Step is a callable (dict → dict) registered by name; run(event) passes the event through each Step in order, short-circuits on StepError (a named exception carrying the step name + original event + error message), returns the transformed dict on success; PipelineRegistry stores named chains by string key; exposes GET /api/v1/pipelines (list), POST /api/v1/pipelines (create: body={name, steps:[{name, transform_type, params}]}), POST /api/v1/pipelines/{name}/run (body={event:{}}), DELETE /api/v1/pipelines/{name}; built-in transform_types: set_field (set key=value), delete_field (remove key), rename_field (old→new), uppercase_field (str upper), lowercase_field (str lower); stdlib only; 20 unit + 10 web tests.
