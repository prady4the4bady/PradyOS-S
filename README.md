...
**Phase 52 — Planned.** Sovereign DistributedLock — named mutex with TTL-based auto-expiry so a crashed holder never deadlocks the system; DistributedLock(name, holder_id, acquired_at, ttl_seconds, expires_at); LockManager.acquire(name, holder_id, ttl=30) -> DistributedLock | None (None if already held by another and not expired); LockManager.release(name, holder_id) -> bool (True only if caller is the current holder); LockManager.refresh(name, holder_id, ttl=30) -> bool (extend TTL); LockManager.is_locked(name) -> bool (True if held and not expired); LockManager.list_locks() -> list[dict] (all active non-expired locks); LockManager.expire_stale() -> int (number of expired locks removed); exposes GET /api/v1/locks (list active locks), POST /api/v1/locks (acquire: body={name, holder_id, ttl?}), DELETE /api/v1/locks/{name} (release: query param holder_id), POST /api/v1/locks/{name}/refresh (refresh: body={holder_id, ttl?}); stdlib only; 20 unit + 10 web tests.
