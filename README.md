...
**Phase 68 — Planned.** Sovereign Scheduler — time-based job queue; each Job has a job_id (uuid4), name, run_at (float epoch), interval_seconds (float|None — None = one-shot, >0 = repeating), payload (dict), status (pending|running|completed|failed|cancelled), last_run_at (float|None), next_run_at (float|None), result (dict|None), error (str|None); Scheduler.schedule(name, run_at, payload, interval=None) → Job; Scheduler.cancel(job_id) → bool; Scheduler.tick(now=None) executes all jobs whose next_run_at <= now (defaults to time.time()), calls registered handler(payload) → dict, records result; repeating jobs reschedule themselves (next_run_at += interval_seconds); Scheduler.list_jobs(status=None, limit=50) most-recently-scheduled first; Scheduler.get(job_id); stores up to 1000 jobs; exposes POST /api/v1/scheduler/jobs, POST /api/v1/scheduler/tick, GET /api/v1/scheduler/jobs, GET /api/v1/scheduler/jobs/{id}, DELETE /api/v1/scheduler/jobs/{id}; stdlib only; thread-safe.
