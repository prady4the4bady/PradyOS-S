...
**Phase 49 — Planned.** Sovereign TaskQueue — priority work queue with worker pool; Task(id=uuid4 hex, name: str, payload: dict, priority: int=5, status: 'pending'|'running'|'done'|'failed', created_at/started_at/finished_at: float|None, result: dict|None, error: str); TaskQueue with submit(name, payload, priority=5)->Task, get(task_id)->Task|None, list_tasks(status=None)->list[Task], cancel(task_id)->bool; WorkerPool(task_queue, num_workers=2, handler: Callable[[Task],dict]) spawns N daemon threads that pull from queue and execute handler; exposes POST /api/v1/tasks, GET /api/v1/tasks, GET /api/v1/tasks/{id}, DELETE /api/v1/tasks/{id} (cancel); stdlib only (threading, queue.PriorityQueue); 20 unit + 10 web tests.
