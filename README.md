...
**Phase 90 — Planned.** Sovereign Quotient Filter — a cache-friendly approximate-membership structure (Bender et al.) that, unlike the Phase 72 Bloom filter, supports **deletion, counting, resizing, and merging**. A fingerprint is split into a *quotient* (the home slot) and a *remainder* (stored in the slot); collisions are resolved by linear probing with three metadata bits (`is_occupied`, `is_continuation`, `is_shifted`) that keep each quotient's run contiguous and ordered. `QuotientFilter(capacity, bits=8)` with `add(item)` / `contains(item)` / `delete(item)` / `merge(other)` / stats, exposed at `/api/v1/quotient`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`; rounds out the membership family alongside Bloom (P72) and Cuckoo (P86) with a counting/mergeable filter.)
