...
**Phase 57 — Planned.** Sovereign SemaphoreGate — named counting semaphores with configurable capacity; SemaphoreGate wraps threading.Semaphore; create(name, capacity=1), acquire(name, timeout=None) → bool (True if acquired, False if timed out), release(name), get_stats(name) returns {name, capacity, available, waiters, acquired_total, released_total, timeout_total}; SemaphoreTimeoutError on acquire(timeout=0 explicit exceeded); exposes GET /api/v1/semaphores (list all + stats), POST /api/v1/semaphores (create: body={name, capacity?}), POST /api/v1/semaphores/{name}/acquire (body={timeout?}), POST /api/v1/semaphores/{name}/release, GET /api/v1/semaphores/{name}; stdlib only (threading.Semaphore); 20 unit + 10 web tests.
