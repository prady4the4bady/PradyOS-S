...
**Phase 89 — Planned.** Sovereign SimHash — a near-duplicate / similarity sketch that maps each item to a `bits`-wide fingerprint by summing per-feature sign vectors and thresholding at zero; two items' **Hamming distance** over their fingerprints approximates their cosine/angular dissimilarity, so near-duplicates share most bits. `SimHash(bits=64)` with `add(name, features)` / `fingerprint(name)` / `distance(a, b)` / `similar(a, threshold)` / stats, exposed at `/api/v1/simhash`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`; complements MinHash — Jaccard/set similarity — with bit-fingerprint Hamming similarity for documents and feature vectors.)
