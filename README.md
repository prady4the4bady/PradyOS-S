...
**Phase 85 — Planned.** Sovereign Reservoir Sampler — Algorithm R for a uniform random sample of fixed size k from a stream of unknown length in a single pass; `ReservoirSampler(k, seed=None)` with `offer(item)` / `sample()` / `count` (items seen) / `reset()`, exposed at `/api/v1/reservoir`; stdlib only (seeded RNG for determinism); thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`.)
