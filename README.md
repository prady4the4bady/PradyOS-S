...
**Phase 81 — Planned.** Sovereign Segment Tree — supports O(log n) range queries (sum / min / max) with point updates over a fixed array, more general than a Fenwick tree; `SegmentTree` with `build(values)` / `update(i, value)` / `query(lo, hi)` / `clear()`, exposed at `/api/v1/segtree`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`.)
