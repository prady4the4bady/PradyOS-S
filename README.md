...
**Phase 39 — Planned.** Sovereign Memory Layer — the OS remembers context across sessions: a MemoryStore holds named memory entries (key, value: any JSON-serialisable, tags: list[str], created_at, updated_at, ttl: float|None for expiry in seconds); store(key, value, tags=[], ttl=None) upserts an entry; recall(key) returns the entry if not expired, None if expired or missing; search(tag) returns all non-expired entries with that tag; forget(key) removes an entry; expire() removes all entries past their ttl; expiry is checked lazily on recall/search, and explicitly via expire(); MemoryStore is optionally persistent via SnapshotStore (namespace='memory'); exposes GET /api/v1/memory/{key} (recall), POST /api/v1/memory/{key} (store: body {value, tags, ttl}), DELETE /api/v1/memory/{key} (forget), GET /api/v1/memory/search?tag=X (search by tag), POST /api/v1/memory/expire (trigger explicit expiry sweep); stdlib only.
