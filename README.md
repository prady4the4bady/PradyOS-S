...
**Phase 79 — Planned.** Sovereign T-Digest — a streaming quantile sketch estimating percentiles (p50 / p95 / p99 …) over a stream from compact, mergeable centroids with error skewed small at the tails; `TDigest` with `add(value, weight=1)` / `quantile(q)` / `cdf(value)` / `merge(other)` / `clear()`, exposed at `/api/v1/tdigest`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`.)
