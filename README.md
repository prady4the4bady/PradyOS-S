...
**Phase 91 — Planned.** Sovereign GK Quantile Sketch (Greenwald–Khanna) — streaming **ε-approximate quantiles** (p50/p95/p99) over an unbounded stream in O((1/ε)·log(εn)) space, with a guaranteed rank error of at most ⌈εn⌉. It keeps a sorted summary of tuples `(value, g, Δ)` where `g` is the gap to the previous retained value and `Δ` bounds the rank uncertainty; an item is inserted and the summary periodically compressed to bound size. `GKQuantile(epsilon=0.01)` with `add(x)` / `quantile(phi)` / `rank(x)` / `merge(other)` / stats, exposed at `/api/v1/quantile`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`. The originally-suggested "T-Digest" already shipped as Phase 79 at `/api/v1/tdigest`, so Phase 91 advances to the Greenwald–Khanna sketch instead — a different, rank-error-guaranteed quantile algorithm that completes the streaming-stats family beside frequency (P76), top-K (P87), and similarity (P88/P89).)
