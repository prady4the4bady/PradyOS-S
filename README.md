...
**Phase 87 — Planned.** Sovereign Top-K (Space-Saving) — a streaming heavy-hitters sketch that tracks the approximate `k` most frequent items in O(k) memory in one pass (Metwally–Agrawal Space-Saving): keep `k` monitored counters; on a miss, evict the minimum-count entry, reassign it to the new item, and carry its old count forward as the error bound. `SpaceSaving(k)` with `add(item)` / `top(n)` / `count(item)` / stats, exposed at `/api/v1/topk`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`; the originally-suggested "Count-Min Sketch" already shipped as Phase 76 at `/api/v1/frequency`, so Phase 87 advances to Space-Saving Top-K instead.)
