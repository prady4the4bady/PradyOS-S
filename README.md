...
**Phase 93 — Planned.** Sovereign Theta Sketch (KMV) — distinct-count (cardinality) estimation via **K-Minimum-Values**: hash each item to `[0,1)` and keep the `k` smallest distinct hashes; the `k`-th smallest, `θ`, estimates the fraction of the hash space covered, so `(k−1)/θ` estimates the distinct count — with **native, lossless set-union mergeability** (the union's KMV is just the `k` smallest hashes of the combined sets). `ThetaSketch(k=4096, seed=0)` with `update(x)` / `estimate()` / `union(other)` / stats, exposed at `/api/v1/theta`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`. **Note:** the originally-suggested "HyperLogLog++" duplicates Phase 74's HyperLogLog cardinality estimator (`pradyos/core/hyperloglog.py`, `/api/v1/cardinality`); Phase 93 advances to the Theta/KMV sketch instead — a *different* cardinality algorithm whose set-union mergeability complements both HLL (P74) and the mergeable quantile sketch (P92).)
