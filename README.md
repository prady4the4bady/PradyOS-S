...
**Phase 77 — Planned.** Sovereign Merkle Tree — a binary hash tree over an ordered list of leaves yielding a single tamper-evident root plus O(log n) inclusion proofs; `MerkleTree` with `add_leaf(data)` / `root()` / `proof(index)` / `verify(data, proof, root)`, exposed at `/api/v1/merkle`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`; the originally-suggested "Bloom Filter" is already shipped as Phase 72 at `/api/v1/bloom`.)
