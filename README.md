...
**Phase 88 — Planned.** Sovereign MinHash — estimates the **Jaccard similarity** of two sets in compact O(num_perm) space by keeping, per hash permutation, the minimum hashed value across a set's members; the fraction of matching minima between two signatures is an unbiased estimate of `|A∩B| / |A∪B|`. `MinHash(num_perm=128)` with `update(item)` / `merge(other)` / `jaccard(other)` / `signature()` / stats, exposed at `/api/v1/minhash`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`; complements the cardinality (HyperLogLog, Phase 74) and frequency (Phases 76/87) sketches with set-*similarity* estimation.)
