...
**Phase 67 — Planned.** Sovereign Process Manager — tracks long-running processes (business process instances, not OS processes); each ProcessInstance has a process_id (uuid4), process_name, state (str, user-defined), context (dict, mutable), history (list of {at, from_state, to_state, trigger, context_snapshot}), created_at, updated_at; ProcessManager.create(name, initial_state, context) → ProcessInstance; .transition(process_id, trigger, new_state, context_patch={}) applies context_patch via shallow merge, records history entry, updates state; .get(process_id) → ProcessInstance | None; .list_processes(state=None, limit=50) → list filtered by state if given, most-recently-updated first; stores up to 500 instances in memory (deque + O(1) index); exposes POST /api/v1/processes (body: {name, state, context}), POST /api/v1/processes/{id}/transition (body: {trigger, state, context_patch}), GET /api/v1/processes/{id}, GET /api/v1/processes?state=X&limit=N; stdlib only; thread-safe.
