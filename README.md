...
**Phase 92 — Planned.** Sovereign KLL Sketch (Karnin–Lang–Liberty, 2016) — the **mergeable, space-optimal** streaming-quantile sketch, proven asymptotically optimal: `O((1/ε)·log·log(1/δ))` space for rank error `εn` with confidence `1−δ`. A hierarchy of capacity-`k` compactors, one per level, each randomly drops every other item on overflow and promotes the rest up a level (halving resolution as weight doubles); levels are sized geometrically so the top dominates the space. `KLLSketch(k=200, seed=0)` with `update(x)` / `quantile(phi)` / `rank(x)` / `merge(other)` / stats, exposed at `/api/v1/kll`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`; the randomized space-optimal successor to Phase 91's deterministic Greenwald–Khanna, and natively mergeable for distributed aggregation.)
