...
**Phase 82 — Planned.** Sovereign Union-Find — a disjoint-set forest with union-by-rank and path compression for near-O(1) connectivity queries; `UnionFind` with `find(x)` / `union(x, y)` / `connected(x, y)` / `count` (number of disjoint sets) / `clear()`, exposed at `/api/v1/unionfind`; stdlib only; thread-safe via `threading.Lock`. (Namespace verified free on `origin/main`.)
