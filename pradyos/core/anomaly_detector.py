...
class AnomalyDetector:
    ...
    def detect(self, signal: str, window: float = 3600.0) -> AnomalyResult:
        ...
        points = [
            p for p in self._aggregator.get(signal, limit=100_000)
        ...
        if n == 0:
            return AnomalyResult(
                signal=signal, sample_size=0, window=window,
                mean=0.0, stddev=0.0, latest_value=0.0,
                z_score=0.0, severity="normal", computed_at=time.time(),
            )
        ...
        if n < 2:
            ...
            return AnomalyResult(
                signal=signal, sample_size=n, window=window,
                mean=round(mean, 6), stddev=0.0,
        ...
        return AnomalyResult(
            ...
            latest_value=round(latest_value, 6),
            z_score=round(z, 6), severity=_severity(z),
