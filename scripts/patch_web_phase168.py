#!/usr/bin/env python3
"""Phase 168 — Patch pradyos/sovereign_web.py (additive, surgical).

The Polygon routes live in ``pradyos/web/polygon_web.py``; this patch wires them into the factory.
Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_polygon_routes after the Phase 167 convex_hull import.
  2. Add     `polygon: Any | None = None,` to create_app() signature.
  3. Insert  `register_polygon_routes(app, polygon)` immediately before `return app`
             (the polygon lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.convex_hull_web import register_convexhull_routes  # Phase 167\n"
NEW_IMPORT = (
...
OLD_PARAM = "    convex_hull: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the polygon router before `return app` ───────────────────────
OLD_RETURN = "    register_convexhull_routes(app, convex_hull)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("polygon router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_polygon_routes" in original:
        print("Already patched — 'register_polygon_routes' present. Nothing to do.")
        return
    ...
    if "register_polygon_routes" not in text:
