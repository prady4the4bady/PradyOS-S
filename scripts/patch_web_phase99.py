#!/usr/bin/env python3
"""Phase 99 — Patch pradyos/sovereign_web.py (additive, surgical).

The Misra-Gries routes live in ``pradyos/web/misra_gries_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file
or touch the DASHBOARD_HTML line:
  1. Import  register_misra_gries_routes after the Phase 98 weighted_reservoir import.
  2. Add     `misra_gries: Any | None = None,` to create_app() signature.
  3. Insert  `register_misra_gries_routes(app, misra_gries)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.weighted_reservoir_web import register_weighted_reservoir_routes  # Phase 98\n"
NEW_IMPORT = (
...
OLD_PARAM = "    weighted_reservoir: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the misra-gries router before `return app` ──────────────────
OLD_RETURN = "    register_weighted_reservoir_routes(app, weighted_reservoir)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("misra-gries router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_misra_gries_routes" in original:
        print("Already patched — 'register_misra_gries_routes' present. Nothing to do.")
        return
    ...
    if "register_misra_gries_routes" not in text:
