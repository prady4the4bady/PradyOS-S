#!/usr/bin/env python3
"""Phase 104 — Patch pradyos/sovereign_web.py (additive, surgical).

The Augmented Sketch routes live in ``pradyos/web/augmentedsketch_web.py``; this
patch wires them into the factory. Three additive edits, none of which rewrite
the file or touch the DASHBOARD_HTML line:
  1. Import  register_augmentedsketch_routes after the Phase 103 spectralbloom import.
  2. Add     `augmented_sketch: Any | None = None,` to create_app() signature.
  3. Insert  `register_augmentedsketch_routes(app, augmented_sketch)` immediately
             before `return app` (the sketch lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.spectralbloom_web import register_spectralbloom_routes  # Phase 103\n"
NEW_IMPORT = (
...
OLD_PARAM = "    spectral_bloom: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the augmented sketch router before `return app` ─────────────
OLD_RETURN = "    register_spectralbloom_routes(app, spectral_bloom)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("augmented sketch router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_augmentedsketch_routes" in original:
        print("Already patched — 'register_augmentedsketch_routes' present. Nothing to do.")
        return
    ...
    if "register_augmentedsketch_routes" not in text:
