#!/usr/bin/env python3
"""Phase 144 — Patch pradyos/sovereign_web.py (additive, surgical).

The Min-Max Heap routes live in ``pradyos/web/min_max_heap_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_minmaxheap_routes after the Phase 143 xortrie import.
  2. Add     `min_max_heap: Any | None = None,` to create_app() signature.
  3. Insert  `register_minmaxheap_routes(app, min_max_heap)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.xor_trie_web import register_xortrie_routes  # Phase 143\n"
NEW_IMPORT = (
...
OLD_PARAM = "    xor_trie: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the minmaxheap router before `return app` ───────────────────
OLD_RETURN = "    register_xortrie_routes(app, xor_trie)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("minmaxheap router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_minmaxheap_routes" in original:
        print("Already patched — 'register_minmaxheap_routes' present. Nothing to do.")
        return
    ...
    if "register_minmaxheap_routes" not in text:
