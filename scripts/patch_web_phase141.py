#!/usr/bin/env python3
"""Phase 141 — Patch pradyos/sovereign_web.py (additive, surgical).

The Suffix Array routes live in ``pradyos/web/suffix_array_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_suffixarray_routes after the Phase 140 radixtree import.
  2. Add     `suffix_array: Any | None = None,` to create_app() signature.
  3. Insert  `register_suffixarray_routes(app, suffix_array)` immediately before `return app`
             (the index lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.radix_tree_web import register_radixtree_routes  # Phase 140\n"
NEW_IMPORT = (
...
OLD_PARAM = "    radix_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the suffixarray router before `return app` ──────────────────
OLD_RETURN = "    register_radixtree_routes(app, radix_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("suffixarray router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_suffixarray_routes" in original:
        print("Already patched — 'register_suffixarray_routes' present. Nothing to do.")
        return
    ...
    if "register_suffixarray_routes" not in text:
