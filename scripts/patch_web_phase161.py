#!/usr/bin/env python3
"""Phase 161 — Patch pradyos/sovereign_web.py (additive, surgical).

The Binary Lifting routes live in ``pradyos/web/binary_lifting_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_binarylifting_routes after the Phase 160 binomial_heap import.
  2. Add     `binary_lifting: Any | None = None,` to create_app() signature.
  3. Insert  `register_binarylifting_routes(app, binary_lifting)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.binomial_heap_web import register_binomial_routes  # Phase 160\n"
NEW_IMPORT = (
...
OLD_PARAM = "    binomial_heap: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the binary_lifting router before `return app` ────────────────
OLD_RETURN = "    register_binomial_routes(app, binomial_heap)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("binary_lifting router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_binarylifting_routes" in original:
        print("Already patched — 'register_binarylifting_routes' present. Nothing to do.")
        return
    ...
    if "register_binarylifting_routes" not in text:
