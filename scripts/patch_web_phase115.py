#!/usr/bin/env python3
"""Phase 115 — Patch pradyos/sovereign_web.py (additive, surgical).

The MinHash LSH routes live in ``pradyos/web/minhash_lsh_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_minhashlsh_routes after the Phase 114 bloomier import.
  2. Add     `minhash_lsh: Any | None = None,` to create_app() signature.
  3. Insert  `register_minhashlsh_routes(app, minhash_lsh)` immediately before
             `return app` (the index lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.bloomier_web import register_bloomier_routes  # Phase 114\n"
NEW_IMPORT = (
...
OLD_PARAM = "    bloomier: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the minhash lsh router before `return app` ──────────────────
OLD_RETURN = "    register_bloomier_routes(app, bloomier)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("minhash lsh router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_minhashlsh_routes" in original:
        print("Already patched — 'register_minhashlsh_routes' present. Nothing to do.")
        return
    ...
    if "register_minhashlsh_routes" not in text:
