#!/usr/bin/env python3
"""Phase 152 — Patch pradyos/sovereign_web.py (additive, surgical).

The van Emde Boas routes live in ``pradyos/web/van_emde_boas_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_veb_routes after the Phase 151 suffix_automaton import.
  2. Add     `van_emde_boas: Any | None = None,` to create_app() signature.
  3. Insert  `register_veb_routes(app, van_emde_boas)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.suffix_automaton_web import register_suffixautomaton_routes  # Phase 151\n"
NEW_IMPORT = (
...
OLD_PARAM = "    suffix_automaton: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the veb router before `return app` ──────────────────────────
OLD_RETURN = "    register_suffixautomaton_routes(app, suffix_automaton)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("veb router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_veb_routes" in original:
        print("Already patched — 'register_veb_routes' present. Nothing to do.")
        return
    ...
    if "register_veb_routes" not in text:
