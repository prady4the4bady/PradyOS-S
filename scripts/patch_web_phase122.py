#!/usr/bin/env python3
"""Phase 122 — Patch pradyos/sovereign_web.py (additive, surgical).

The b-bit MinHash routes live in ``pradyos/web/bbit_minhash_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_bbitminhash_routes after the Phase 121 iblt import.
  2. Add     `bbit_minhash: Any | None = None,` to create_app() signature.
  3. Insert  `register_bbitminhash_routes(app, bbit_minhash)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.iblt_web import register_iblt_routes  # Phase 121\n"
NEW_IMPORT = (
...
# ── Edit 3: register the bbit minhash router before `return app` ─────────────────
OLD_RETURN = "    register_iblt_routes(app, iblt)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("bbit minhash router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_bbitminhash_routes" in original:
        print("Already patched — 'register_bbitminhash_routes' present. Nothing to do.")
        return
    ...
    if "register_bbitminhash_routes" not in text:
