#!/usr/bin/env python3
"""Phase 128 — Patch pradyos/sovereign_web.py (additive, surgical).

The Golomb-Coded Set routes live in ``pradyos/web/gcs_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_gcs_routes after the Phase 127 random projection import.
  2. Add     `gcs: Any | None = None,` to create_app() signature.
  3. Insert  `register_gcs_routes(app, gcs)` immediately before `return app`
             (the set lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.random_projection_web import register_randomprojection_routes  # Phase 127\n"
NEW_IMPORT = (
...
OLD_PARAM = "    random_projection: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the gcs router before `return app` ──────────────────────────
OLD_RETURN = "    register_randomprojection_routes(app, random_projection)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("gcs router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_gcs_routes" in original:
        print("Already patched — 'register_gcs_routes' present. Nothing to do.")
        return
    ...
    if "register_gcs_routes" not in text:
