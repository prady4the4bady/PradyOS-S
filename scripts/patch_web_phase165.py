#!/usr/bin/env python3
"""Phase 165 — Patch pradyos/sovereign_web.py (additive, surgical).

The Heavy-Light Decomposition routes live in ``pradyos/web/heavy_light_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_hld_routes after the Phase 164 ternary_search_tree import.
  2. Add     `heavy_light: Any | None = None,` to create_app() signature.
  3. Insert  `register_hld_routes(app, heavy_light)` immediately before `return app`
             (the structure lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.ternary_search_tree_web import register_tst_routes  # Phase 164\n"
NEW_IMPORT = (
...
OLD_PARAM = "    ternary_search_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the hld router before `return app` ───────────────────────────
OLD_RETURN = "    register_tst_routes(app, ternary_search_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("hld router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_hld_routes" in original:
        print("Already patched — 'register_hld_routes' present. Nothing to do.")
        return
    ...
    if "register_hld_routes" not in text:
