#!/usr/bin/env python3
"""Phase 94 — Patch pradyos/sovereign_web.py (additive, surgical).

The Count Sketch routes live in ``pradyos/web/count_sketch_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file
or touch the DASHBOARD_HTML line:
  1. Import  register_count_sketch_routes after the Phase 93 theta import.
  2. Add     `count_sketch: Any | None = None,` to create_app() signature.
  3. Insert  `register_count_sketch_routes(app, count_sketch)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.theta_sketch_web import register_theta_sketch_routes  # Phase 93\n"
NEW_IMPORT = (
...
# ── Edit 3: register the count sketch router before `return app` ─────────────────
OLD_RETURN = "    register_theta_sketch_routes(app, theta)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("count sketch router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_count_sketch_routes" in original:
        print("Already patched — 'register_count_sketch_routes' present. Nothing to do.")
        return
    ...
    if "register_count_sketch_routes" not in text:
