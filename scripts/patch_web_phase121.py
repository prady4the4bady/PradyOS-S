#!/usr/bin/env python3
"""Phase 121 — Patch pradyos/sovereign_web.py (additive, surgical).

The IBLT routes live in ``pradyos/web/iblt_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_iblt_routes after the Phase 120 maglev import.
  2. Add     `iblt: Any | None = None,` to create_app() signature.
  3. Insert  `register_iblt_routes(app, iblt)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.maglev_web import register_maglev_routes  # Phase 120\n"
NEW_IMPORT = (
...
OLD_PARAM = "    maglev: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the iblt router before `return app` ─────────────────────────
OLD_RETURN = "    register_maglev_routes(app, maglev)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("iblt router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_iblt_routes" in original:
        print("Already patched — 'register_iblt_routes' present. Nothing to do.")
        return
    ...
    if "register_iblt_routes" not in text:
