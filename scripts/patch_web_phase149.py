#!/usr/bin/env python3
"""Phase 149 — Patch pradyos/sovereign_web.py (additive, surgical).

The Persistent Segment Tree routes live in ``pradyos/web/persistent_segment_tree_web.py``; this
patch wires them into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_perseg_routes after the Phase 148 li_chao_tree import.
  2. Add     `persistent_segment_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_perseg_routes(app, persistent_segment_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.li_chao_tree_web import register_lichao_routes  # Phase 148\n"
NEW_IMPORT = (
...
OLD_PARAM = "    li_chao_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the perseg router before `return app` ───────────────────────
OLD_RETURN = "    register_lichao_routes(app, li_chao_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("perseg router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_perseg_routes" in original:
        print("Already patched — 'register_perseg_routes' present. Nothing to do.")
        return
    ...
    if "register_perseg_routes" not in text:
