#!/usr/bin/env python3
"""Phase 114 — Patch pradyos/sovereign_web.py (additive, surgical).

The Bloomier Filter routes live in ``pradyos/web/bloomier_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_bloomier_routes after the Phase 113 treap import.
  2. Add     `bloomier: Any | None = None,` to create_app() signature.
  3. Insert  `register_bloomier_routes(app, bloomier)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.treap_web import register_treap_routes  # Phase 113\n"
NEW_IMPORT = (
...
# ── Edit 3: register the bloomier router before `return app` ─────────────────────
OLD_RETURN = "    register_treap_routes(app, treap)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("bloomier router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_bloomier_routes" in original:
        print("Already patched — 'register_bloomier_routes' present. Nothing to do.")
        return
    ...
    if "register_bloomier_routes" not in text:
