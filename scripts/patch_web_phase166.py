#!/usr/bin/env python3
"""Phase 166 — Patch pradyos/sovereign_web.py (additive, surgical).

The Sparse Segment Tree routes live in ``pradyos/web/sparse_segment_tree_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_sparseseg_routes after the Phase 165 heavy_light import.
  2. Add     `sparse_segment_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_sparseseg_routes(app, sparse_segment_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.heavy_light_web import register_hld_routes  # Phase 165\n"
NEW_IMPORT = (
...
OLD_PARAM = "    heavy_light: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the sparseseg router before `return app` ─────────────────────
OLD_RETURN = "    register_hld_routes(app, heavy_light)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("sparseseg router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_sparseseg_routes" in original:
        print("Already patched — 'register_sparseseg_routes' present. Nothing to do.")
        return
    ...
    if "register_sparseseg_routes" not in text:
