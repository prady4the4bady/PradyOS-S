#!/usr/bin/env python3
"""Phase 89 — Patch pradyos/sovereign_web.py (additive, surgical).

The SimHash routes live in ``pradyos/web/simhash_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_simhash_routes after the Phase 88 minhash import.
  2. Add     `simhash: Any | None = None,` to create_app() signature.
  3. Insert  `register_simhash_routes(app, simhash)` immediately before
             `return app` (the store lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.minhash_web import register_minhash_routes  # Phase 88\n"
NEW_IMPORT = (
...
OLD_PARAM = "    minhash: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the simhash router before `return app` ──────────────────────
OLD_RETURN = "    register_minhash_routes(app, minhash)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("simhash router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_simhash_routes" in original:
        print("Already patched — 'register_simhash_routes' present. Nothing to do.")
        return
    ...
    if "register_simhash_routes" not in text:
