#!/usr/bin/env python3
"""Phase 106 — Patch pradyos/sovereign_web.py (additive, surgical).

The Moment Sketch routes live in ``pradyos/web/moment_sketch_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file
or touch the DASHBOARD_HTML line:
  1. Import  register_momentsketch_routes after the Phase 105 qdigest import.
  2. Add     `moment_sketch: Any | None = None,` to create_app() signature.
  3. Insert  `register_momentsketch_routes(app, moment_sketch)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.qdigest_web import register_qdigest_routes  # Phase 105\n"
NEW_IMPORT = (
...
OLD_PARAM = "    qdigest: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the moment sketch router before `return app` ────────────────
OLD_RETURN = "    register_qdigest_routes(app, qdigest)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("moment sketch router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_momentsketch_routes" in original:
        print("Already patched — 'register_momentsketch_routes' present. Nothing to do.")
        return
    ...
    if "register_momentsketch_routes" not in text:
