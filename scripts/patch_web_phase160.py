#!/usr/bin/env python3
"""Phase 160 — Patch pradyos/sovereign_web.py (additive, surgical).

The Binomial Heap routes live in ``pradyos/web/binomial_heap_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_binomial_routes after the Phase 159 scapegoat_tree import.
  2. Add     `binomial_heap: Any | None = None,` to create_app() signature.
  3. Insert  `register_binomial_routes(app, binomial_heap)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.scapegoat_tree_web import register_scapegoat_routes  # Phase 159\n"
NEW_IMPORT = (
...
OLD_PARAM = "    scapegoat_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the binomial router before `return app` ──────────────────────
OLD_RETURN = "    register_scapegoat_routes(app, scapegoat_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("binomial router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_binomial_routes" in original:
        print("Already patched — 'register_binomial_routes' present. Nothing to do.")
        return
    ...
    if "register_binomial_routes" not in text:
