#!/usr/bin/env python3
"""Phase 95 — Patch pradyos/sovereign_web.py (additive, surgical).

The Lossy Counting routes live in ``pradyos/web/lossy_count_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file
or touch the DASHBOARD_HTML line:
  1. Import  register_lossy_count_routes after the Phase 94 count_sketch import.
  2. Add     `lossy: Any | None = None,` to create_app() signature.
  3. Insert  `register_lossy_count_routes(app, lossy)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.count_sketch_web import register_count_sketch_routes  # Phase 94\n"
NEW_IMPORT = (
...
OLD_PARAM = "    count_sketch: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the lossy count router before `return app` ──────────────────
OLD_RETURN = "    register_count_sketch_routes(app, count_sketch)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("lossy count router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_lossy_count_routes" in original:
        print("Already patched — 'register_lossy_count_routes' present. Nothing to do.")
        return
    ...
    if "register_lossy_count_routes" not in text:
