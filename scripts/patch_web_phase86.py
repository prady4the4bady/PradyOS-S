#!/usr/bin/env python3
"""Phase 86 — Patch pradyos/sovereign_web.py (additive, surgical).

The cuckoo routes live in ``pradyos/web/cuckoo_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_cuckoo_routes after the Phase 85 reservoir import.
  2. Add     `cuckoo: Any | None = None,` to create_app() signature.
  3. Insert  `register_cuckoo_routes(app, cuckoo)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.reservoir_web import register_reservoir_routes  # Phase 85\n"
NEW_IMPORT = (
...
OLD_PARAM = "    reservoir: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the cuckoo router before `return app` ───────────────────────
OLD_RETURN = "    register_reservoir_routes(app, reservoir)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("cuckoo router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_cuckoo_routes" in original:
        print("Already patched — 'register_cuckoo_routes' present. Nothing to do.")
        return
    ...
    if "register_cuckoo_routes" not in text:
