#!/usr/bin/env python3
"""Phase 136 — Patch pradyos/sovereign_web.py (additive, surgical).

The Skew Heap routes live in ``pradyos/web/skew_heap_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_skewheap_routes after the Phase 135 wavelet import.
  2. Add     `skew_heap: Any | None = None,` to create_app() signature.
  3. Insert  `register_skewheap_routes(app, skew_heap)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.wavelet_tree_web import register_wavelet_routes  # Phase 135\n"
NEW_IMPORT = (
...
OLD_PARAM = "    wavelet_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the skewheap router before `return app` ─────────────────────
OLD_RETURN = "    register_wavelet_routes(app, wavelet_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("skewheap router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_skewheap_routes" in original:
        print("Already patched — 'register_skewheap_routes' present. Nothing to do.")
        return
    ...
    if "register_skewheap_routes" not in text:
