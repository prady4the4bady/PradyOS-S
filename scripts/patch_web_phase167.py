#!/usr/bin/env python3
"""Phase 167 — Patch pradyos/sovereign_web.py (additive, surgical).

The Convex Hull routes live in ``pradyos/web/convex_hull_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_convexhull_routes after the Phase 166 sparse_segment_tree import.
  2. Add     `convex_hull: Any | None = None,` to create_app() signature.
  3. Insert  `register_convexhull_routes(app, convex_hull)` immediately before `return app`
             (the hull lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.sparse_segment_tree_web import register_sparseseg_routes  # Phase 166\n"
NEW_IMPORT = (
...
OLD_PARAM = "    sparse_segment_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the convexhull router before `return app` ────────────────────
OLD_RETURN = "    register_sparseseg_routes(app, sparse_segment_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("convexhull router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_convexhull_routes" in original:
        print("Already patched — 'register_convexhull_routes' present. Nothing to do.")
        return
    ...
    if "register_convexhull_routes" not in text:
