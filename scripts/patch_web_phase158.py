#!/usr/bin/env python3
"""Phase 158 — Patch pradyos/sovereign_web.py (additive, surgical).

The Leftist Heap routes live in ``pradyos/web/leftist_heap_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_leftist_routes after the Phase 157 range_tree import.
  2. Add     `leftist_heap: Any | None = None,` to create_app() signature.
  3. Insert  `register_leftist_routes(app, leftist_heap)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.range_tree_web import register_rangetree_routes  # Phase 157\n"
NEW_IMPORT = (
...
OLD_PARAM = "    range_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the leftist router before `return app` ───────────────────────
OLD_RETURN = "    register_rangetree_routes(app, range_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("leftist router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_leftist_routes" in original:
        print("Already patched — 'register_leftist_routes' present. Nothing to do.")
        return
    ...
    if "register_leftist_routes" not in text:
