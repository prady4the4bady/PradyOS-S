#!/usr/bin/env python3
"""Phase 96 — Patch pradyos/sovereign_web.py (additive, surgical).

The DDSketch routes live in ``pradyos/web/ddsketch_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_ddsketch_routes after the Phase 95 lossy_count import.
  2. Add     `ddsketch: Any | None = None,` to create_app() signature.
  3. Insert  `register_ddsketch_routes(app, ddsketch)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.lossy_count_web import register_lossy_count_routes  # Phase 95\n"
NEW_IMPORT = (
...
# ── Edit 3: register the ddsketch router before `return app` ─────────────────────
OLD_RETURN = "    register_lossy_count_routes(app, lossy)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("ddsketch router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_ddsketch_routes" in original:
        print("Already patched — 'register_ddsketch_routes' present. Nothing to do.")
        return
    ...
    if "register_ddsketch_routes" not in text:
