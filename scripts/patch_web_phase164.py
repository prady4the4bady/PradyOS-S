#!/usr/bin/env python3
"""Phase 164 — Patch pradyos/sovereign_web.py (additive, surgical).

The Ternary Search Tree routes live in ``pradyos/web/ternary_search_tree_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_tst_routes after the Phase 163 lazy_segment_tree import.
  2. Add     `ternary_search_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_tst_routes(app, ternary_search_tree)` immediately before `return app`
             (the table lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.lazy_segment_tree_web import register_lazyseg_routes  # Phase 163\n"
NEW_IMPORT = (
...
OLD_PARAM = "    lazy_segment_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the tst router before `return app` ───────────────────────────
OLD_RETURN = "    register_lazyseg_routes(app, lazy_segment_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("tst router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_tst_routes" in original:
        print("Already patched — 'register_tst_routes' present. Nothing to do.")
        return
    ...
    if "register_tst_routes" not in text:
