#!/usr/bin/env python3
"""Phase 88 — Patch pradyos/sovereign_web.py (additive, surgical).

The MinHash routes live in ``pradyos/web/minhash_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_minhash_routes after the Phase 87 topk import.
  2. Add     `minhash: Any | None = None,` to create_app() signature.
  3. Insert  `register_minhash_routes(app, minhash)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.topk_web import register_topk_routes  # Phase 87\n"
NEW_IMPORT = (
...
OLD_PARAM = "    space_saving: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the minhash router before `return app` ──────────────────────
OLD_RETURN = "    register_topk_routes(app, space_saving)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("minhash router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_minhash_routes" in original:
        print("Already patched — 'register_minhash_routes' present. Nothing to do.")
        return
    ...
    if "register_minhash_routes" not in text:
