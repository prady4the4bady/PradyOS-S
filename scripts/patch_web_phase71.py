#!/usr/bin/env python3
"""Phase 71 — Patch pradyos/sovereign_web.py (additive, surgical).

Three edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  AnomalyWatch + SourceNotFoundError after the Phase 70 deps import.
  2. Add     `anomaly_watch: Any | None = None,` to create_app() signature.
  3. Insert  the /api/v1/anomaly/* endpoints immediately before `return app`.
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.core.dependency_graph import DependencyGraph, CycleError  # Phase 70\n"
NEW_IMPORT = (
...
OLD_PARAM = "    dependency_graph: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: /api/v1/anomaly/* endpoints before `return app` ──────────────────────
ANOMALY_ROUTES = '''    @app.get("/api/v1/anomaly/sources")
...
def patch() -> None:
    ...
    if "anomaly_watch" in original:
        print("Already patched — 'anomaly_watch' present. Nothing to do.")
        return
    ...
    if "anomaly_watch" not in text or "/api/v1/anomaly/sources" not in text:
