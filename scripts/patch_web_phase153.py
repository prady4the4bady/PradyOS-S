#!/usr/bin/env python3
"""Phase 153 — Patch pradyos/sovereign_web.py (additive, surgical).

The PR Quadtree routes live in ``pradyos/web/pr_quadtree_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_pr_quadtree_routes after the Phase 152 van_emde_boas import.
  2. Add     `pr_quadtree: Any | None = None,` to create_app() signature.
  3. Insert  `register_pr_quadtree_routes(app, pr_quadtree)` immediately before `return app`
             (the quadtree lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.van_emde_boas_web import register_veb_routes  # Phase 152\n"
NEW_IMPORT = (
...
OLD_PARAM = "    van_emde_boas: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the pr_quadtree router before `return app` ───────────────────
OLD_RETURN = "    register_veb_routes(app, van_emde_boas)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("pr_quadtree router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_pr_quadtree_routes" in original:
        print("Already patched — 'register_pr_quadtree_routes' present. Nothing to do.")
        return
    ...
    if "register_pr_quadtree_routes" not in text:
