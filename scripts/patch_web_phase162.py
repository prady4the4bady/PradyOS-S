#!/usr/bin/env python3
"""Phase 162 — Patch pradyos/sovereign_web.py (additive, surgical).

The Implicit Treap routes live in ``pradyos/web/implicit_treap_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_implicittreap_routes after the Phase 161 binary_lifting import.
  2. Add     `implicit_treap: Any | None = None,` to create_app() signature.
  3. Insert  `register_implicittreap_routes(app, implicit_treap)` immediately before `return app`
             (the sequence lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.binary_lifting_web import register_binarylifting_routes  # Phase 161\n"
NEW_IMPORT = (
...
OLD_PARAM = "    binary_lifting: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the implicit_treap router before `return app` ────────────────
OLD_RETURN = "    register_binarylifting_routes(app, binary_lifting)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("implicit_treap router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_implicittreap_routes" in original:
        print("Already patched — 'register_implicittreap_routes' present. Nothing to do.")
        return
    ...
    if "register_implicittreap_routes" not in text:
