#!/usr/bin/env python3
"""Phase 98 — Patch pradyos/sovereign_web.py (additive, surgical).

The Weighted Reservoir routes live in ``pradyos/web/weighted_reservoir_web.py``;
this patch wires them into the factory. Three additive edits, none of which
rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_weighted_reservoir_routes after the Phase 97 exp-histogram import.
  2. Add     `weighted_reservoir: Any | None = None,` to create_app() signature.
  3. Insert  `register_weighted_reservoir_routes(app, weighted_reservoir)` immediately
             before `return app` (the sampler lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.exponential_histogram_web import register_exponential_histogram_routes  # Phase 97\n"
NEW_IMPORT = (
...
OLD_PARAM = "    exp_histogram: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the weighted reservoir router before `return app` ────────────
OLD_RETURN = "    register_exponential_histogram_routes(app, exp_histogram)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("weighted reservoir router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_weighted_reservoir_routes" in original:
        print("Already patched — 'register_weighted_reservoir_routes' present. Nothing to do.")
        return
    ...
    if "register_weighted_reservoir_routes" not in text:
