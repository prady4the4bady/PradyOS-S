#!/usr/bin/env python3
"""Phase 93 — Patch pradyos/sovereign_web.py (additive, surgical).

The Theta sketch routes live in ``pradyos/web/theta_sketch_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file
or touch the DASHBOARD_HTML line:
  1. Import  register_theta_sketch_routes after the Phase 92 kll import.
  2. Add     `theta: Any | None = None,` to create_app() signature.
  3. Insert  `register_theta_sketch_routes(app, theta)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.kll_sketch_web import register_kll_sketch_routes  # Phase 92\n"
NEW_IMPORT = (
...
# ── Edit 3: register the theta sketch router before `return app` ─────────────────
OLD_RETURN = "    register_kll_sketch_routes(app, kll)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("theta sketch router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_theta_sketch_routes" in original:
        print("Already patched — 'register_theta_sketch_routes' present. Nothing to do.")
        return
    ...
    if "register_theta_sketch_routes" not in text:
