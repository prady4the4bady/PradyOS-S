#!/usr/bin/env python3
"""Phase 92 — Patch pradyos/sovereign_web.py (additive, surgical).

The KLL sketch routes live in ``pradyos/web/kll_sketch_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_kll_sketch_routes after the Phase 91 gk_quantile import.
  2. Add     `kll: Any | None = None,` to create_app() signature.
  3. Insert  `register_kll_sketch_routes(app, kll)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.gk_quantile_web import register_gk_quantile_routes  # Phase 91\n"
NEW_IMPORT = (
...
OLD_PARAM = "    gk_quantile: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the kll sketch router before `return app` ───────────────────
OLD_RETURN = "    register_gk_quantile_routes(app, gk_quantile)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("kll sketch router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_kll_sketch_routes" in original:
        print("Already patched — 'register_kll_sketch_routes' present. Nothing to do.")
        return
    ...
    if "register_kll_sketch_routes" not in text:
