#!/usr/bin/env python3
"""Phase 112 — Patch pradyos/sovereign_web.py (additive, surgical).

The Linear Counting routes live in ``pradyos/web/linear_counter_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_linearcounting_routes after the Phase 111 morris import.
  2. Add     `linear_counter: Any | None = None,` to create_app() signature.
  3. Insert  `register_linearcounting_routes(app, linear_counter)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.morris_web import register_morris_routes  # Phase 111\n"
NEW_IMPORT = (
...
OLD_PARAM = "    morris_counter: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the linear counting router before `return app` ──────────────
OLD_RETURN = "    register_morris_routes(app, morris_counter)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("linear counting router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_linearcounting_routes" in original:
        print("Already patched — 'register_linearcounting_routes' present. Nothing to do.")
        return
    ...
    if "register_linearcounting_routes" not in text:
