#!/usr/bin/env python3
"""Phase 108 — Patch pradyos/sovereign_web.py (additive, surgical).

The Binary Fuse routes live in ``pradyos/web/binary_fuse_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file
or touch the DASHBOARD_HTML line:
  1. Import  register_binaryfuse_routes after the Phase 107 counting_bloom import.
  2. Add     `binary_fuse: Any | None = None,` to create_app() signature.
  3. Insert  `register_binaryfuse_routes(app, binary_fuse)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.counting_bloom_web import register_countingbloom_routes  # Phase 107\n"
NEW_IMPORT = (
...
OLD_PARAM = "    counting_bloom: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the binary fuse router before `return app` ──────────────────
OLD_RETURN = "    register_countingbloom_routes(app, counting_bloom)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("binary fuse router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_binaryfuse_routes" in original:
        print("Already patched — 'register_binaryfuse_routes' present. Nothing to do.")
        return
    ...
    if "register_binaryfuse_routes" not in text:
