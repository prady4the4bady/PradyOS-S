#!/usr/bin/env python3
"""Phase 126 — Patch pradyos/sovereign_web.py (additive, surgical).

The Cosine/SimHash-LSH routes live in ``pradyos/web/simhash_lsh_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_simhashlsh_routes after the Phase 125 frugal import.
  2. Add     `simhash_lsh: Any | None = None,` to create_app() signature.
  3. Insert  `register_simhashlsh_routes(app, simhash_lsh)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.frugal_web import register_frugal_routes  # Phase 125\n"
NEW_IMPORT = (
...
OLD_PARAM = "    frugal: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the simhash lsh router before `return app` ──────────────────
OLD_RETURN = "    register_frugal_routes(app, frugal)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("simhash lsh router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_simhashlsh_routes" in original:
        print("Already patched — 'register_simhashlsh_routes' present. Nothing to do.")
        return
    ...
    if "register_simhashlsh_routes" not in text:
