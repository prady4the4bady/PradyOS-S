#!/usr/bin/env python3
"""Phase 163 — Patch pradyos/sovereign_web.py (additive, surgical).

The Lazy Segment Tree routes live in ``pradyos/web/lazy_segment_tree_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_lazyseg_routes after the Phase 162 implicit_treap import.
  2. Add     `lazy_segment_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_lazyseg_routes(app, lazy_segment_tree)` immediately before `return app`
             (the tree lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.implicit_treap_web import register_implicittreap_routes  # Phase 162\n"
NEW_IMPORT = (
...
OLD_PARAM = "    implicit_treap: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the lazyseg router before `return app` ───────────────────────
OLD_RETURN = "    register_implicittreap_routes(app, implicit_treap)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("lazyseg router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_lazyseg_routes" in original:
        print("Already patched — 'register_lazyseg_routes' present. Nothing to do.")
        return
    ...
    if "register_lazyseg_routes" not in text:
