#!/usr/bin/env python3
"""Phase 133 — Patch pradyos/sovereign_web.py (additive, surgical).

The Splay Tree routes live in ``pradyos/web/splay_tree_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_splaytree_routes after the Phase 132 cuckoohash import.
  2. Add     `splay_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_splaytree_routes(app, splay_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.cuckoohash_web import register_cuckoohash_routes  # Phase 132\n"
NEW_IMPORT = (
...
OLD_PARAM = "    cuckoo_hashtable: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the splaytree router before `return app` ────────────────────
OLD_RETURN = "    register_cuckoohash_routes(app, cuckoo_hashtable)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("splaytree router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_splaytree_routes" in original:
        print("Already patched — 'register_splaytree_routes' present. Nothing to do.")
        return
    ...
    if "register_splaytree_routes" not in text:
