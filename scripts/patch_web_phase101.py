#!/usr/bin/env python3
"""Phase 101 — Patch pradyos/sovereign_web.py (additive, surgical).

The Ribbon Filter routes live in ``pradyos/web/ribbon_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_ribbon_routes after the Phase 100 xor-filter import.
  2. Add     `ribbon_filter: Any | None = None,` to create_app() signature.
  3. Insert  `register_ribbon_routes(app, ribbon_filter)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.xor_filter_web import register_xor_filter_routes  # Phase 100\n"
NEW_IMPORT = (
...
OLD_PARAM = "    xor_filter: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the ribbon filter router before `return app` ────────────────
OLD_RETURN = "    register_xor_filter_routes(app, xor_filter)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("ribbon filter router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_ribbon_routes" in original:
        print("Already patched — 'register_ribbon_routes' present. Nothing to do.")
        return
    ...
    if "register_ribbon_routes" not in text:
