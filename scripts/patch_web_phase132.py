#!/usr/bin/env python3
"""Phase 132 — Patch pradyos/sovereign_web.py (additive, surgical).

The Cuckoo Hash Table routes live in ``pradyos/web/cuckoohash_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_cuckoohash_routes after the Phase 131 priority-sampling import.
  2. Add     `cuckoo_hashtable: Any | None = None,` to create_app() signature.
  3. Insert  `register_cuckoohash_routes(app, cuckoo_hashtable)` immediately before
             `return app` (the table lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.priority_sampling_web import register_prioritysample_routes  # Phase 131\n"
NEW_IMPORT = (
...
OLD_PARAM = "    priority_sample: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the cuckoohash router before `return app` ───────────────────
OLD_RETURN = "    register_prioritysample_routes(app, priority_sample)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("cuckoohash router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_cuckoohash_routes" in original:
        print("Already patched — 'register_cuckoohash_routes' present. Nothing to do.")
        return
    ...
    if "register_cuckoohash_routes" not in text:
