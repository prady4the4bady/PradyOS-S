#!/usr/bin/env python3
"""Phase 83 — Patch pradyos/sovereign_web.py (additive, surgical).

Unlike earlier phases (which inlined their routes), Phase 83 keeps the route
definitions in their own module, ``pradyos/web/trie_web.py``, and merely wires
them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_trie_routes after the Phase 82 unionfind import.
  2. Add     `trie: Any | None = None,` to create_app() signature.
  3. Insert  `register_trie_routes(app, trie)` immediately before `return app`
             (the trie instance lives in factory scope — no module-level state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.core.unionfind import UnionFind  # Phase 82\n"
NEW_IMPORT = (
...
OLD_PARAM = "    unionfind: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the trie router before `return app` ─────────────────────────
OLD_RETURN = (
...
EDITS = [
    ...
    ("trie router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_trie_routes" in original:
        print("Already patched — 'register_trie_routes' present. Nothing to do.")
        return
    ...
    if "register_trie_routes" not in text:
