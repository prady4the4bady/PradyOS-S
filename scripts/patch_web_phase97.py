#!/usr/bin/env python3
"""Phase 97 — Patch pradyos/sovereign_web.py (additive, surgical).

The Exponential Histogram routes live in ``pradyos/web/exponential_histogram_web.py``;
this patch wires them into the factory. Three additive edits, none of which rewrite
the file or touch the DASHBOARD_HTML line:
  1. Import  register_exponential_histogram_routes after the Phase 96 ddsketch import.
  2. Add     `exp_histogram: Any | None = None,` to create_app() signature.
  3. Insert  `register_exponential_histogram_routes(app, exp_histogram)` immediately
             before `return app` (the histogram lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.ddsketch_web import register_ddsketch_routes  # Phase 96\n"
NEW_IMPORT = (
...
OLD_PARAM = "    ddsketch: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the exponential histogram router before `return app` ─────────
OLD_RETURN = "    register_ddsketch_routes(app, ddsketch)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("exponential histogram router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_exponential_histogram_routes" in original:
        print("Already patched — 'register_exponential_histogram_routes' present. Nothing to do.")
        return
    ...
    if "register_exponential_histogram_routes" not in text:
