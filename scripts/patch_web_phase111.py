#!/usr/bin/env python3
"""Phase 111 — Patch pradyos/sovereign_web.py (additive, surgical).

The Morris Counter routes live in ``pradyos/web/morris_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_morris_routes after the Phase 110 stable_bloom import.
  2. Add     `morris_counter: Any | None = None,` to create_app() signature.
  3. Insert  `register_morris_routes(app, morris_counter)` immediately before
             `return app` (the counter lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.stable_bloom_web import register_stablebloom_routes  # Phase 110\n"
NEW_IMPORT = (
...
OLD_PARAM = "    stable_bloom: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the morris counter router before `return app` ───────────────
OLD_RETURN = "    register_stablebloom_routes(app, stable_bloom)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("morris router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_morris_routes" in original:
        print("Already patched — 'register_morris_routes' present. Nothing to do.")
        return
    ...
    if "register_morris_routes" not in text:
