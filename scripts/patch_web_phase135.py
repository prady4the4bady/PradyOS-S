#!/usr/bin/env python3
"""Phase 135 — Patch pradyos/sovereign_web.py (additive, surgical).

The Wavelet Tree routes live in ``pradyos/web/wavelet_tree_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_wavelet_routes after the Phase 134 rankselect import.
  2. Add     `wavelet_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_wavelet_routes(app, wavelet_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.rank_select_web import register_rankselect_routes  # Phase 134\n"
NEW_IMPORT = (
...
OLD_PARAM = "    rank_select: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the wavelet router before `return app` ──────────────────────
OLD_RETURN = "    register_rankselect_routes(app, rank_select)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("wavelet router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_wavelet_routes" in original:
        print("Already patched — 'register_wavelet_routes' present. Nothing to do.")
        return
    ...
    if "register_wavelet_routes" not in text:
