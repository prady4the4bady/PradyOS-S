#!/usr/bin/env python3
"""Phase 84 — Patch pradyos/sovereign_web.py (additive, surgical).

The LRU routes live in their own module (``pradyos/web/lru_web.py``); this patch
merely wires them into the factory. Three additive edits, none of which rewrite
the file or touch the DASHBOARD_HTML line:
  1. Import  register_lru_routes after the Phase 83 trie import.
  2. Add     `lru_cache: Any | None = None,` to create_app() signature.
  3. Insert  `register_lru_routes(app, lru_cache)` immediately before `return app`
             (the cache lives in factory scope — no module-level state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.trie_web import register_trie_routes  # Phase 83\n"
NEW_IMPORT = (
...
# ── Edit 3: register the LRU router before `return app` ──────────────────────────
OLD_RETURN = "    register_trie_routes(app, trie)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("lru router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_lru_routes" in original:
        print("Already patched — 'register_lru_routes' present. Nothing to do.")
        return
    ...
    if "register_lru_routes" not in text:
