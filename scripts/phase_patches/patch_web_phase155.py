#!/usr/bin/env python3
"""Phase 155 — Patch pradyos/sovereign_web.py (additive, surgical).

The AVL Tree routes live in ``pradyos/web/avl_tree_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_avl_routes after the Phase 154 fibonacci_heap import.
  2. Add     `avl_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_avl_routes(app, avl_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.fibonacci_heap_web import register_fibonacci_routes  # Phase 154\n"
NEW_IMPORT = (
...
OLD_PARAM = "    fibonacci_heap: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the avl router before `return app` ───────────────────────────
OLD_RETURN = "    register_fibonacci_routes(app, fibonacci_heap)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("avl router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_avl_routes" in original:
        print("Already patched — 'register_avl_routes' present. Nothing to do.")
        return
    ...
    if "register_avl_routes" not in text:
