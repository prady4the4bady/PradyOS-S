#!/usr/bin/env python3
"""Phase 131 — Patch pradyos/sovereign_web.py (additive, surgical).

The Priority Sampling routes live in ``pradyos/web/priority_sampling_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_prioritysample_routes after the Phase 130 ams import.
  2. Add     `priority_sample: Any | None = None,` to create_app() signature.
  3. Insert  `register_prioritysample_routes(app, priority_sample)` immediately before
             `return app` (the sample lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.ams_web import register_ams_routes  # Phase 130\n"
NEW_IMPORT = (
...
# ── Edit 3: register the priority-sampling router before `return app` ────────────
OLD_RETURN = "    register_ams_routes(app, ams)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("priority-sampling router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_prioritysample_routes" in original:
        print("Already patched — 'register_prioritysample_routes' present. Nothing to do.")
        return
    ...
    if "register_prioritysample_routes" not in text:
