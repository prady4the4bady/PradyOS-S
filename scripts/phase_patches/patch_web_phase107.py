#!/usr/bin/env python3
"""Phase 107 — Patch pradyos/sovereign_web.py (additive, surgical).

The Counting Bloom routes live in ``pradyos/web/counting_bloom_web.py``; this
patch wires them into the factory. Three additive edits, none of which rewrite
the file or touch the DASHBOARD_HTML line:
  1. Import  register_countingbloom_routes after the Phase 106 moment_sketch import.
  2. Add     `counting_bloom: Any | None = None,` to create_app() signature.
  3. Insert  `register_countingbloom_routes(app, counting_bloom)` immediately
             before `return app` (the filter lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.moment_sketch_web import register_momentsketch_routes  # Phase 106\n"
NEW_IMPORT = (
...
OLD_PARAM = "    moment_sketch: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the counting bloom router before `return app` ───────────────
OLD_RETURN = "    register_momentsketch_routes(app, moment_sketch)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("counting bloom router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_countingbloom_routes" in original:
        print("Already patched — 'register_countingbloom_routes' present. Nothing to do.")
        return
    ...
    if "register_countingbloom_routes" not in text:
