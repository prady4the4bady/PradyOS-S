#!/usr/bin/env python3
"""Phase 151 — Patch pradyos/sovereign_web.py (additive, surgical).

The Suffix Automaton routes live in ``pradyos/web/suffix_automaton_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_suffixautomaton_routes after the Phase 150 pairing_heap import.
  2. Add     `suffix_automaton: Any | None = None,` to create_app() signature.
  3. Insert  `register_suffixautomaton_routes(app, suffix_automaton)` immediately before `return app`
             (the automaton lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.pairing_heap_web import register_pairingheap_routes  # Phase 150\n"
NEW_IMPORT = (
...
OLD_PARAM = "    pairing_heap: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the suffixautomaton router before `return app` ──────────────
OLD_RETURN = "    register_pairingheap_routes(app, pairing_heap)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("suffixautomaton router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_suffixautomaton_routes" in original:
        print("Already patched — 'register_suffixautomaton_routes' present. Nothing to do.")
        return
    ...
    if "register_suffixautomaton_routes" not in text:
