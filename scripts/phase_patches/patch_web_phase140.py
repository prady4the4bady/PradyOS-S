#!/usr/bin/env python3
"""Phase 140 — Patch pradyos/sovereign_web.py (additive, surgical).

The Radix Tree routes live in ``pradyos/web/radix_tree_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_radixtree_routes after the Phase 139 kdtree import.
  2. Add     `radix_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_radixtree_routes(app, radix_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.kd_tree_web import register_kdtree_routes  # Phase 139\n"
NEW_IMPORT = (
...
OLD_PARAM = "    kd_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the radixtree router before `return app` ────────────────────
OLD_RETURN = "    register_kdtree_routes(app, kd_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("radixtree router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_radixtree_routes" in original:
        print("Already patched — 'register_radixtree_routes' present. Nothing to do.")
        return
    ...
    if "register_radixtree_routes" not in text:
