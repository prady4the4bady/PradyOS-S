#!/usr/bin/env python3
"""Phase 125 — Patch pradyos/sovereign_web.py (additive, surgical).

The Frugal quantile routes live in ``pradyos/web/frugal_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_frugal_routes after the Phase 124 jump import.
  2. Add     `frugal: Any | None = None,` to create_app() signature.
  3. Insert  `register_frugal_routes(app, frugal)` immediately before `return app`
             (the estimator lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.jump_web import register_jump_routes  # Phase 124\n"
NEW_IMPORT = (
...
OLD_PARAM = "    jump_hash: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the frugal router before `return app` ───────────────────────
OLD_RETURN = "    register_jump_routes(app, jump_hash)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("frugal router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_frugal_routes" in original:
        print("Already patched — 'register_frugal_routes' present. Nothing to do.")
        return
    ...
    if "register_frugal_routes" not in text:
