#!/usr/bin/env python3
"""Phase 130 — Patch pradyos/sovereign_web.py (additive, surgical).

The AMS / tug-of-war routes live in ``pradyos/web/ams_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_ams_routes after the Phase 129 fmsketch import.
  2. Add     `ams: Any | None = None,` to create_app() signature.
  3. Insert  `register_ams_routes(app, ams)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.fmsketch_web import register_fmsketch_routes  # Phase 129\n"
NEW_IMPORT = (
...
OLD_PARAM = "    fm_sketch: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the ams router before `return app` ──────────────────────────
OLD_RETURN = "    register_fmsketch_routes(app, fm_sketch)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("ams router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_ams_routes" in original:
        print("Already patched — 'register_ams_routes' present. Nothing to do.")
        return
    ...
    if "register_ams_routes" not in text:
