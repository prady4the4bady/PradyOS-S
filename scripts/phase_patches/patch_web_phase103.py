#!/usr/bin/env python3
"""Phase 103 — Patch pradyos/sovereign_web.py (additive, surgical).

The Spectral Bloom Filter routes live in ``pradyos/web/spectralbloom_web.py``;
...
rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_spectralbloom_routes after the Phase 102 heavykeeper import.
  2. Add     `spectral_bloom: Any | None = None,` to create_app() signature.
  3. Insert  `register_spectralbloom_routes(app, spectral_bloom)` immediately
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.heavykeeper_web import register_heavykeeper_routes  # Phase 102\n"
NEW_IMPORT = (
...
OLD_PARAM = "    heavykeeper: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the spectral bloom router before `return app` ───────────────
OLD_RETURN = "    register_heavykeeper_routes(app, heavykeeper)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("spectral bloom router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_spectralbloom_routes" in original:
        print("Already patched — 'register_spectralbloom_routes' present. Nothing to do.")
        return
    ...
    if "register_spectralbloom_routes" not in text:
