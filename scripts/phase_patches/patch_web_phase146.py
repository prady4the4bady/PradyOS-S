#!/usr/bin/env python3
"""Phase 146 — Patch pradyos/sovereign_web.py (additive, surgical).

The 2D Fenwick routes live in ``pradyos/web/fenwick2d_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_fenwick2d_routes after the Phase 145 cartesiantree import.
  2. Add     `fenwick2d: Any | None = None,` to create_app() signature.
  3. Insert  `register_fenwick2d_routes(app, fenwick2d)` immediately before `return app`
             (the grid lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.cartesian_tree_web import register_cartesiantree_routes  # Phase 145\n"
NEW_IMPORT = (
...
OLD_PARAM = "    cartesian_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the fenwick2d router before `return app` ────────────────────
OLD_RETURN = "    register_cartesiantree_routes(app, cartesian_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("fenwick2d router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_fenwick2d_routes" in original:
        print("Already patched — 'register_fenwick2d_routes' present. Nothing to do.")
        return
    ...
    if "register_fenwick2d_routes" not in text:
