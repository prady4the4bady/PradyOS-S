#!/usr/bin/env python3
"""Phase 85 — Patch pradyos/sovereign_web.py (additive, surgical).

The reservoir routes live in ``pradyos/web/reservoir_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_reservoir_routes after the Phase 84 lru import.
  2. Add     `reservoir: Any | None = None,` to create_app() signature.
  3. Insert  `register_reservoir_routes(app, reservoir)` immediately before
             `return app` (the sampler lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.lru_web import register_lru_routes  # Phase 84\n"
NEW_IMPORT = (
...
OLD_PARAM = "    lru_cache: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the reservoir router before `return app` ────────────────────
OLD_RETURN = "    register_lru_routes(app, lru_cache)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("reservoir router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_reservoir_routes" in original:
        print("Already patched — 'register_reservoir_routes' present. Nothing to do.")
        return
    ...
    if "register_reservoir_routes" not in text:
