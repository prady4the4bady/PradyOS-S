#!/usr/bin/env python3
"""Phase 110 — Patch pradyos/sovereign_web.py (additive, surgical).

The Stable Bloom Filter routes live in ``pradyos/web/stable_bloom_web.py``; this
patch wires them into the factory. Three additive edits, none of which rewrite the
file or touch the DASHBOARD_HTML line:
  1. Import  register_stablebloom_routes after the Phase 109 vacuum import.
  2. Add     `stable_bloom: Any | None = None,` to create_app() signature.
  3. Insert  `register_stablebloom_routes(app, stable_bloom)` immediately before
             `return app` (the filter lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.vacuum_web import register_vacuum_routes  # Phase 109\n"
NEW_IMPORT = (
...
OLD_PARAM = "    vacuum_filter: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the stable bloom router before `return app` ─────────────────
OLD_RETURN = "    register_vacuum_routes(app, vacuum_filter)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("stable bloom router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_stablebloom_routes" in original:
        print("Already patched — 'register_stablebloom_routes' present. Nothing to do.")
        return
    ...
    if "register_stablebloom_routes" not in text:
