#!/usr/bin/env python3
"""Phase 118 — Patch pradyos/sovereign_web.py (additive, surgical).

The Scalable Bloom routes live in ``pradyos/web/scalable_bloom_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_scalablebloom_routes after the Phase 117 hyperminhash import.
  2. Add     `scalable_bloom: Any | None = None,` to create_app() signature.
  3. Insert  `register_scalablebloom_routes(app, scalable_bloom)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.hyper_minhash_web import register_hyperminhash_routes  # Phase 117\n"
NEW_IMPORT = (
...
OLD_PARAM = "    hyper_minhash: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the scalable bloom router before `return app` ───────────────
OLD_RETURN = "    register_hyperminhash_routes(app, hyper_minhash)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("scalable bloom router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_scalablebloom_routes" in original:
        print("Already patched — 'register_scalablebloom_routes' present. Nothing to do.")
        return
    ...
    if "register_scalablebloom_routes" not in text:
