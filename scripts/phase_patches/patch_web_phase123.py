#!/usr/bin/env python3
"""Phase 123 — Patch pradyos/sovereign_web.py (additive, surgical).

The CU-Sketch routes live in ``pradyos/web/cu_sketch_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_cusketch_routes after the Phase 122 bbit minhash import.
  2. Add     `cu_sketch: Any | None = None,` to create_app() signature.
  3. Insert  `register_cusketch_routes(app, cu_sketch)` immediately before `return app`
             (the sketch lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.bbit_minhash_web import register_bbitminhash_routes  # Phase 122\n"
NEW_IMPORT = (
...
OLD_PARAM = "    bbit_minhash: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the cu sketch router before `return app` ────────────────────
OLD_RETURN = "    register_bbitminhash_routes(app, bbit_minhash)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("cu sketch router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_cusketch_routes" in original:
        print("Already patched — 'register_cusketch_routes' present. Nothing to do.")
        return
    ...
    if "register_cusketch_routes" not in text:
