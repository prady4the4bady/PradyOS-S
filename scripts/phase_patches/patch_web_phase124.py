#!/usr/bin/env python3
"""Phase 124 — Patch pradyos/sovereign_web.py (additive, surgical).

The Jump Consistent Hash routes live in ``pradyos/web/jump_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_jump_routes after the Phase 123 cu sketch import.
  2. Add     `jump_hash: Any | None = None,` to create_app() signature.
  3. Insert  `register_jump_routes(app, jump_hash)` immediately before `return app`
             (the state lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.cu_sketch_web import register_cusketch_routes  # Phase 123\n"
NEW_IMPORT = (
...
OLD_PARAM = "    cu_sketch: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the jump hash router before `return app` ────────────────────
OLD_RETURN = "    register_cusketch_routes(app, cu_sketch)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("jump router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_jump_routes" in original:
        print("Already patched — 'register_jump_routes' present. Nothing to do.")
        return
    ...
    if "register_jump_routes" not in text:
