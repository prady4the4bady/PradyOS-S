#!/usr/bin/env python3
"""Phase 143 — Patch pradyos/sovereign_web.py (additive, surgical).

The Binary/XOR Trie routes live in ``pradyos/web/xor_trie_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_xortrie_routes after the Phase 142 ahocorasick import.
  2. Add     `xor_trie: Any | None = None,` to create_app() signature.
  3. Insert  `register_xortrie_routes(app, xor_trie)` immediately before `return app`
             (the trie lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.aho_corasick_web import register_ahocorasick_routes  # Phase 142\n"
NEW_IMPORT = (
...
OLD_PARAM = "    aho_corasick: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the xortrie router before `return app` ──────────────────────
OLD_RETURN = "    register_ahocorasick_routes(app, aho_corasick)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("xortrie router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_xortrie_routes" in original:
        print("Already patched — 'register_xortrie_routes' present. Nothing to do.")
        return
    ...
    if "register_xortrie_routes" not in text:
