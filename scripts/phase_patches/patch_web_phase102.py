#!/usr/bin/env python3
"""Phase 102 — Patch pradyos/sovereign_web.py (additive, surgical).

The HeavyKeeper routes live in ``pradyos/web/heavykeeper_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file
or touch the DASHBOARD_HTML line:
  1. Import  register_heavykeeper_routes after the Phase 101 ribbon import.
  2. Add     `heavykeeper: Any | None = None,` to create_app() signature.
  3. Insert  `register_heavykeeper_routes(app, heavykeeper)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.ribbon_web import register_ribbon_routes  # Phase 101\n"
NEW_IMPORT = (
...
OLD_PARAM = "    ribbon_filter: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the heavykeeper router before `return app` ──────────────────
OLD_RETURN = "    register_ribbon_routes(app, ribbon_filter)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("heavykeeper router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_heavykeeper_routes" in original:
        print("Already patched — 'register_heavykeeper_routes' present. Nothing to do.")
        return
    ...
    if "register_heavykeeper_routes" not in text:
