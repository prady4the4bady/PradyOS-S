#!/usr/bin/env python3
"""Phase 100 — Patch pradyos/sovereign_web.py (additive, surgical).  🎯 centennial.

The XOR Filter routes live in ``pradyos/web/xor_filter_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_xor_filter_routes after the Phase 99 misra-gries import.
  2. Add     `xor_filter: Any | None = None,` to create_app() signature.
  3. Insert  `register_xor_filter_routes(app, xor_filter)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.misra_gries_web import register_misra_gries_routes  # Phase 99\n"
NEW_IMPORT = (
...
OLD_PARAM = "    misra_gries: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the xor filter router before `return app` ───────────────────
OLD_RETURN = "    register_misra_gries_routes(app, misra_gries)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("xor filter router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_xor_filter_routes" in original:
        print("Already patched — 'register_xor_filter_routes' present. Nothing to do.")
        return
    ...
    if "register_xor_filter_routes" not in text:
