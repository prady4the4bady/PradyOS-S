#!/usr/bin/env python3
"""Phase 156 — Patch pradyos/sovereign_web.py (additive, surgical).

The B-Tree routes live in ``pradyos/web/b_tree_web.py``; this patch wires them into the factory.
Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_btree_routes after the Phase 155 avl_tree import.
  2. Add     `b_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_btree_routes(app, b_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.avl_tree_web import register_avl_routes  # Phase 155\n"
NEW_IMPORT = (
...
OLD_PARAM = "    avl_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the btree router before `return app` ─────────────────────────
OLD_RETURN = "    register_avl_routes(app, avl_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("btree router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_btree_routes" in original:
        print("Already patched — 'register_btree_routes' present. Nothing to do.")
        return
    ...
    if "register_btree_routes" not in text:
