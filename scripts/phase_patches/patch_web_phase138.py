#!/usr/bin/env python3
"""Phase 138 — Patch pradyos/sovereign_web.py (additive, surgical).

The Sparse Table routes live in ``pradyos/web/sparse_table_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_sparsetable_routes after the Phase 137 intervaltree import.
  2. Add     `sparse_table: Any | None = None,` to create_app() signature.
  3. Insert  `register_sparsetable_routes(app, sparse_table)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.interval_tree_web import register_intervaltree_routes  # Phase 137\n"
NEW_IMPORT = (
...
OLD_PARAM = "    interval_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the sparsetable router before `return app` ──────────────────
OLD_RETURN = "    register_intervaltree_routes(app, interval_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("sparsetable router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_sparsetable_routes" in original:
        print("Already patched — 'register_sparsetable_routes' present. Nothing to do.")
        return
    ...
    if "register_sparsetable_routes" not in text:
