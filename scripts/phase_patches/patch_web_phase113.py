#!/usr/bin/env python3
"""Phase 113 — Patch pradyos/sovereign_web.py (additive, surgical).

The Treap routes live in ``pradyos/web/treap_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_treap_routes after the Phase 112 linear counting import.
  2. Add     `treap: Any | None = None,` to create_app() signature.
  3. Insert  `register_treap_routes(app, treap)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.linear_counter_web import register_linearcounting_routes  # Phase 112\n"
NEW_IMPORT = (
...
OLD_PARAM = "    linear_counter: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the treap router before `return app` ────────────────────────
OLD_RETURN = "    register_linearcounting_routes(app, linear_counter)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("treap router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_treap_routes" in original:
        print("Already patched — 'register_treap_routes' present. Nothing to do.")
        return
    ...
    if "register_treap_routes" not in text:
