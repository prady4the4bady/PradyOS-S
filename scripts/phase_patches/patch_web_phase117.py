#!/usr/bin/env python3
"""Phase 117 — Patch pradyos/sovereign_web.py (additive, surgical).

The HyperMinHash routes live in ``pradyos/web/hyper_minhash_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_hyperminhash_routes after the Phase 116 tinylfu import.
  2. Add     `hyper_minhash: Any | None = None,` to create_app() signature.
  3. Insert  `register_hyperminhash_routes(app, hyper_minhash)` immediately before
             `return app` (the sketch lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.tiny_lfu_web import register_tinylfu_routes  # Phase 116\n"
NEW_IMPORT = (
...
OLD_PARAM = "    tiny_lfu: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the hyperminhash router before `return app` ─────────────────
OLD_RETURN = "    register_tinylfu_routes(app, tiny_lfu)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("hyperminhash router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_hyperminhash_routes" in original:
        print("Already patched — 'register_hyperminhash_routes' present. Nothing to do.")
        return
    ...
    if "register_hyperminhash_routes" not in text:
