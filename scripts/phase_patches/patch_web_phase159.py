#!/usr/bin/env python3
"""Phase 159 — Patch pradyos/sovereign_web.py (additive, surgical).

The Scapegoat Tree routes live in ``pradyos/web/scapegoat_tree_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_scapegoat_routes after the Phase 158 leftist_heap import.
  2. Add     `scapegoat_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_scapegoat_routes(app, scapegoat_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.leftist_heap_web import register_leftist_routes  # Phase 158\n"
NEW_IMPORT = (
...
OLD_PARAM = "    leftist_heap: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the scapegoat router before `return app` ─────────────────────
OLD_RETURN = "    register_leftist_routes(app, leftist_heap)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("scapegoat router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_scapegoat_routes" in original:
        print("Already patched — 'register_scapegoat_routes' present. Nothing to do.")
        return
    ...
    if "register_scapegoat_routes" not in text:
