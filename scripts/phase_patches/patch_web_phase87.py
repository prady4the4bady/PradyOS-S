#!/usr/bin/env python3
"""Phase 87 — Patch pradyos/sovereign_web.py (additive, surgical).

The Top-K routes live in ``pradyos/web/topk_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_topk_routes after the Phase 86 cuckoo import.
  2. Add     `space_saving: Any | None = None,` to create_app() signature.
  3. Insert  `register_topk_routes(app, space_saving)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.cuckoo_web import register_cuckoo_routes  # Phase 86\n"
NEW_IMPORT = (
...
OLD_PARAM = "    cuckoo: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the topk router before `return app` ─────────────────────────
OLD_RETURN = "    register_cuckoo_routes(app, cuckoo)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("topk router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_topk_routes" in original:
        print("Already patched — 'register_topk_routes' present. Nothing to do.")
        return
    ...
    if "register_topk_routes" not in text:
