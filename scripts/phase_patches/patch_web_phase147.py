#!/usr/bin/env python3
"""Phase 147 — Patch pradyos/sovereign_web.py (additive, surgical).

The Sqrt Decomposition routes live in ``pradyos/web/sqrt_decomposition_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_sqrtdecomp_routes after the Phase 146 fenwick2d import.
  2. Add     `sqrt_decomposition: Any | None = None,` to create_app() signature.
  3. Insert  `register_sqrtdecomp_routes(app, sqrt_decomposition)` immediately before `return app`
             (the array lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.fenwick2d_web import register_fenwick2d_routes  # Phase 146\n"
NEW_IMPORT = (
...
OLD_PARAM = "    fenwick2d: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the sqrtdecomp router before `return app` ───────────────────
OLD_RETURN = "    register_fenwick2d_routes(app, fenwick2d)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("sqrtdecomp router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_sqrtdecomp_routes" in original:
        print("Already patched — 'register_sqrtdecomp_routes' present. Nothing to do.")
        return
    ...
    if "register_sqrtdecomp_routes" not in text:
