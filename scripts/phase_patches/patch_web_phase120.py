#!/usr/bin/env python3
"""Phase 120 — Patch pradyos/sovereign_web.py (additive, surgical).

The Maglev Hashing routes live in ``pradyos/web/maglev_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_maglev_routes after the Phase 119 rendezvous import.
  2. Add     `maglev: Any | None = None,` to create_app() signature.
  3. Insert  `register_maglev_routes(app, maglev)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.rendezvous_web import register_rendezvous_routes  # Phase 119\n"
NEW_IMPORT = (
...
OLD_PARAM = "    rendezvous: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the maglev router before `return app` ───────────────────────
OLD_RETURN = "    register_rendezvous_routes(app, rendezvous)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("maglev router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_maglev_routes" in original:
        print("Already patched — 'register_maglev_routes' present. Nothing to do.")
        return
    ...
    if "register_maglev_routes" not in text:
