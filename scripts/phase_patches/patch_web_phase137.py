#!/usr/bin/env python3
"""Phase 137 — Patch pradyos/sovereign_web.py (additive, surgical).

The Interval Tree routes live in ``pradyos/web/interval_tree_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_intervaltree_routes after the Phase 136 skewheap import.
  2. Add     `interval_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_intervaltree_routes(app, interval_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.skew_heap_web import register_skewheap_routes  # Phase 136\n"
NEW_IMPORT = (
...
OLD_PARAM = "    skew_heap: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the intervaltree router before `return app` ─────────────────
OLD_RETURN = "    register_skewheap_routes(app, skew_heap)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("intervaltree router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_intervaltree_routes" in original:
        print("Already patched — 'register_intervaltree_routes' present. Nothing to do.")
        return
    ...
    if "register_intervaltree_routes" not in text:
