#!/usr/bin/env python3
"""Phase 139 — Patch pradyos/sovereign_web.py (additive, surgical).

The KD-Tree routes live in ``pradyos/web/kd_tree_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_kdtree_routes after the Phase 138 sparsetable import.
  2. Add     `kd_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_kdtree_routes(app, kd_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.sparse_table_web import register_sparsetable_routes  # Phase 138\n"
NEW_IMPORT = (
...
OLD_PARAM = "    sparse_table: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the kdtree router before `return app` ───────────────────────
OLD_RETURN = "    register_sparsetable_routes(app, sparse_table)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("kdtree router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_kdtree_routes" in original:
        print("Already patched — 'register_kdtree_routes' present. Nothing to do.")
        return
    ...
    if "register_kdtree_routes" not in text:
