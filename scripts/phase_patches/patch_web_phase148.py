#!/usr/bin/env python3
"""Phase 148 — Patch pradyos/sovereign_web.py (additive, surgical).

The Li Chao Tree routes live in ``pradyos/web/li_chao_tree_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_lichao_routes after the Phase 147 sqrt_decomposition import.
  2. Add     `li_chao_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_lichao_routes(app, li_chao_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.sqrt_decomposition_web import register_sqrtdecomp_routes  # Phase 147\n"
NEW_IMPORT = (
...
OLD_PARAM = "    sqrt_decomposition: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the lichao router before `return app` ───────────────────────
OLD_RETURN = "    register_sqrtdecomp_routes(app, sqrt_decomposition)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("lichao router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_lichao_routes" in original:
        print("Already patched — 'register_lichao_routes' present. Nothing to do.")
        return
    ...
    if "register_lichao_routes" not in text:
