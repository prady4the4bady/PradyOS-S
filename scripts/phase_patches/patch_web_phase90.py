#!/usr/bin/env python3
"""Phase 90 — Patch pradyos/sovereign_web.py (additive, surgical).

The Quotient Filter routes live in ``pradyos/web/quotient_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file
or touch the DASHBOARD_HTML line:
  1. Import  register_quotient_routes after the Phase 89 simhash import.
  2. Add     `quotient: Any | None = None,` to create_app() signature.
  3. Insert  `register_quotient_routes(app, quotient)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.simhash_web import register_simhash_routes  # Phase 89\n"
NEW_IMPORT = (
...
OLD_PARAM = "    simhash: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the quotient router before `return app` ─────────────────────
OLD_RETURN = "    register_simhash_routes(app, simhash)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("quotient router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_quotient_routes" in original:
        print("Already patched — 'register_quotient_routes' present. Nothing to do.")
        return
    ...
    if "register_quotient_routes" not in text:
