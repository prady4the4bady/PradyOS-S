#!/usr/bin/env python3
"""Phase 157 — Patch pradyos/sovereign_web.py (additive, surgical).

The Range Tree routes live in ``pradyos/web/range_tree_web.py``; this patch wires them into the
factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML line:
  1. Import  register_rangetree_routes after the Phase 156 b_tree import.
  2. Add     `range_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_rangetree_routes(app, range_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.b_tree_web import register_btree_routes  # Phase 156\n"
NEW_IMPORT = (
...
OLD_PARAM = "    b_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the rangetree router before `return app` ─────────────────────
OLD_RETURN = "    register_btree_routes(app, b_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("rangetree router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_rangetree_routes" in original:
        print("Already patched — 'register_rangetree_routes' present. Nothing to do.")
        return
    ...
    if "register_rangetree_routes" not in text:
