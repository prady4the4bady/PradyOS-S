#!/usr/bin/env python3
"""Phase 127 — Patch pradyos/sovereign_web.py (additive, surgical).

The Random Projection routes live in ``pradyos/web/random_projection_web.py``; this patch
wires them into the factory. Three additive edits, none of which rewrite the file or touch
the DASHBOARD_HTML line:
  1. Import  register_randomprojection_routes after the Phase 126 simhash lsh import.
  2. Add     `random_projection: Any | None = None,` to create_app() signature.
  3. Insert  `register_randomprojection_routes(app, random_projection)` immediately before
             `return app` (the matrix lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.simhash_lsh_web import register_simhashlsh_routes  # Phase 126\n"
NEW_IMPORT = (
...
OLD_PARAM = "    simhash_lsh: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the random projection router before `return app` ────────────
OLD_RETURN = "    register_simhashlsh_routes(app, simhash_lsh)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("random projection router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_randomprojection_routes" in original:
        print("Already patched — 'register_randomprojection_routes' present. Nothing to do.")
        return
    ...
    if "register_randomprojection_routes" not in text:
