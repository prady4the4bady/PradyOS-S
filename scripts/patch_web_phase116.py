#!/usr/bin/env python3
"""Phase 116 — Patch pradyos/sovereign_web.py (additive, surgical).

The TinyLFU routes live in ``pradyos/web/tiny_lfu_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_tinylfu_routes after the Phase 115 minhash lsh import.
  2. Add     `tiny_lfu: Any | None = None,` to create_app() signature.
  3. Insert  `register_tinylfu_routes(app, tiny_lfu)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.minhash_lsh_web import register_minhashlsh_routes  # Phase 115\n"
NEW_IMPORT = (
...
OLD_PARAM = "    minhash_lsh: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the tinylfu router before `return app` ──────────────────────
OLD_RETURN = "    register_minhashlsh_routes(app, minhash_lsh)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("tinylfu router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_tinylfu_routes" in original:
        print("Already patched — 'register_tinylfu_routes' present. Nothing to do.")
        return
    ...
    if "register_tinylfu_routes" not in text:
