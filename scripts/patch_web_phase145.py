#!/usr/bin/env python3
"""Phase 145 — Patch pradyos/sovereign_web.py (additive, surgical).

The Cartesian Tree routes live in ``pradyos/web/cartesian_tree_web.py``; this patch wires them
into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_cartesiantree_routes after the Phase 144 minmaxheap import.
  2. Add     `cartesian_tree: Any | None = None,` to create_app() signature.
  3. Insert  `register_cartesiantree_routes(app, cartesian_tree)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.min_max_heap_web import register_minmaxheap_routes  # Phase 144\n"
NEW_IMPORT = (
...
OLD_PARAM = "    min_max_heap: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the cartesiantree router before `return app` ─────────────────
OLD_RETURN = "    register_minmaxheap_routes(app, min_max_heap)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("cartesiantree router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_cartesiantree_routes" in original:
        print("Already patched — 'register_cartesiantree_routes' present. Nothing to do.")
        return
    ...
    if "register_cartesiantree_routes" not in text:
