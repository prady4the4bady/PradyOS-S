#!/usr/bin/env python3
"""Phase 129 — Patch pradyos/sovereign_web.py (additive, surgical).

The Flajolet–Martin / PCSA routes live in ``pradyos/web/fmsketch_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or touch the
DASHBOARD_HTML line:
  1. Import  register_fmsketch_routes after the Phase 128 gcs import.
  2. Add     `fm_sketch: Any | None = None,` to create_app() signature.
  3. Insert  `register_fmsketch_routes(app, fm_sketch)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.gcs_web import register_gcs_routes  # Phase 128\n"
NEW_IMPORT = (
...
# ── Edit 3: register the fmsketch router before `return app` ─────────────────────
OLD_RETURN = "    register_gcs_routes(app, gcs)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("fmsketch router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_fmsketch_routes" in original:
        print("Already patched — 'register_fmsketch_routes' present. Nothing to do.")
        return
    ...
    if "register_fmsketch_routes" not in text:
