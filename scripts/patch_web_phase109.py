#!/usr/bin/env python3
"""Phase 109 — Patch pradyos/sovereign_web.py (additive, surgical).

The Vacuum Filter routes live in ``pradyos/web/vacuum_web.py``; this patch wires
them into the factory. Three additive edits, none of which rewrite the file or
touch the DASHBOARD_HTML line:
  1. Import  register_vacuum_routes after the Phase 108 binary_fuse import.
  2. Add     `vacuum_filter: Any | None = None,` to create_app() signature.
  3. Insert  `register_vacuum_routes(app, vacuum_filter)` immediately before
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.binary_fuse_web import register_binaryfuse_routes  # Phase 108\n"
NEW_IMPORT = (
...
OLD_PARAM = "    binary_fuse: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the vacuum filter router before `return app` ────────────────
OLD_RETURN = "    register_binaryfuse_routes(app, binary_fuse)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("vacuum router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_vacuum_routes" in original:
        print("Already patched — 'register_vacuum_routes' present. Nothing to do.")
        return
    ...
    if "register_vacuum_routes" not in text:
