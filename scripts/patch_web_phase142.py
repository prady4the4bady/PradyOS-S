#!/usr/bin/env python3
"""Phase 142 — Patch pradyos/sovereign_web.py (additive, surgical).

The Aho-Corasick routes live in ``pradyos/web/aho_corasick_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_ahocorasick_routes after the Phase 141 suffixarray import.
  2. Add     `aho_corasick: Any | None = None,` to create_app() signature.
  3. Insert  `register_ahocorasick_routes(app, aho_corasick)` immediately before `return app`
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.suffix_array_web import register_suffixarray_routes  # Phase 141\n"
NEW_IMPORT = (
...
OLD_PARAM = "    suffix_array: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the ahocorasick router before `return app` ──────────────────
OLD_RETURN = "    register_suffixarray_routes(app, suffix_array)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("ahocorasick router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_ahocorasick_routes" in original:
        print("Already patched — 'register_ahocorasick_routes' present. Nothing to do.")
        return
    ...
    if "register_ahocorasick_routes" not in text:
