#!/usr/bin/env python3
"""Phase 134 — Patch pradyos/sovereign_web.py (additive, surgical).

The Rank/Select routes live in ``pradyos/web/rank_select_web.py``; this patch wires them into
the factory. Three additive edits, none of which rewrite the file or touch the DASHBOARD_HTML
line:
  1. Import  register_rankselect_routes after the Phase 133 splaytree import.
  2. Add     `rank_select: Any | None = None,` to create_app() signature.
  3. Insert  `register_rankselect_routes(app, rank_select)` immediately before `return app`
             (the bitvector lives in factory scope — no module state).
...
# ── Edit 1: import ──────────────────────────────────────────────────────────────
OLD_IMPORT = "from pradyos.web.splay_tree_web import register_splaytree_routes  # Phase 133\n"
NEW_IMPORT = (
...
OLD_PARAM = "    splay_tree: Any | None = None,\n) -> FastAPI:"
NEW_PARAM = (
...
# ── Edit 3: register the rankselect router before `return app` ───────────────────
OLD_RETURN = "    register_splaytree_routes(app, splay_tree)\n\n    return app"
NEW_RETURN = (
...
EDITS = [
    ...
    ("rankselect router wiring", OLD_RETURN, NEW_RETURN),
]
...
def patch() -> None:
    ...
    if "register_rankselect_routes" in original:
        print("Already patched — 'register_rankselect_routes' present. Nothing to do.")
        return
    ...
    if "register_rankselect_routes" not in text:
